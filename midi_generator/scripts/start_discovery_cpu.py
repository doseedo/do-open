#!/usr/bin/env python
"""
Start discovery pipeline using CPU multiprocessing.
...
Usage:
    python scripts/start_discovery_cpu.py --iterations 12 --cores 56
"""
...
def load_midi_corpus_as_numpy(corpus_path: str, max_files: int = None):
    ...
    print(f"Loading {len(file_paths)} MIDI files...")
    ...
    for i, path in enumerate(file_paths):
        try:
            ...
            print(f"  [!] Warning: Failed to load {os.path.basename(path)}: {str(e)}")
    ...
    for i, midi in enumerate(midi_files):
        try:
            ...
            if (i + 1) % 100 == 0:
                print(f"  Converted {i+1}/{len(midi_files)} files...")

        except Exception as e:
            print(f"  [!] Warning: Failed to convert file: {str(e)}")
...
def load_initial_transforms():
    ...
    print(f"✓ Loaded {len(primitives)} irreducible primitives")
...
def main():
    ...
    print(f"\n{'='*70}")
    print("NEURAL PROGRAM SYNTHESIS - CPU DISCOVERY PIPELINE")
    print(f"{'='*70}\n")
