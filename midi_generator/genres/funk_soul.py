...
- Giorgio Moroder production techniques (Munich sound, "I Feel Love")
- Nile Rodgers guitar analysis (Chic rhythmic approach)
...
- Disco four-on-the-floor drum patterns (Chic, Donna Summer)
- Disco string sections (orchestral stabs and sustained pads)
- Eurodisco sequenced synth arpeggios (Giorgio Moroder)
- Nu-disco modern production (Daft Punk, Jamiroquai)
- Nile Rodgers rhythmic guitar technique
...
class FunkStyle(Enum):
    ...
    CLASSIC_DISCO = "classic_disco"  # Chic, Sister Sledge, Donna Summer
    EURODISCO = "eurodisco"  # Giorgio Moroder, ABBA, Boney M
    ITALO_DISCO = "italo_disco"  # Gazebo, Baltimora, Ryan Paris
    NU_DISCO = "nu_disco"  # Daft Punk, Jamiroquai, Parcels
...
class FunkSoulGenerator:
    ...
    def generate_funk_groove(self,
        ...
        Now also supports disco styles with four-on-the-floor patterns and
        ...
        Args:
            ...
            style: Funk/Disco style (James Brown, Parliament, Classic Disco, etc.)

        Returns:
            FunkGroove with drums, bass, guitar, keys, and horns
        """
        # Delegate to disco groove generator for disco styles
        if style in [FunkStyle.CLASSIC_DISCO, FunkStyle.EURODISCO,
                     FunkStyle.ITALO_DISCO, FunkStyle.NU_DISCO]:
            return self.generate_disco_groove(measures=measures, style=style)

        # Original funk implementation
    ...
    def generate_disco_drums(self,
                            measures: int = 4,
                            style: FunkStyle = FunkStyle.CLASSIC_DISCO,
                            open_hihat_density: float = 0.8) -> List[DrumHit]:
        """
        Generate disco drum patterns with four-on-the-floor

        The disco drum pattern is characterized by the "four-on-the-floor" kick
        drum (kick on every quarter note), which became the defining rhythmic
        foundation of disco. As documented in "Turn the Beat Around", this pattern
        emerged from the fusion of funk, soul, and European electronic music.

        Args:
            measures: Number of measures
            style: Disco style (Classic, Eurodisco, Italo, Nu-Disco)
            open_hihat_density: Frequency of open hi-hat hits (0.0-1.0)

        Returns:
            List of drum hits with four-on-the-floor kick pattern
        """
        hits = []
        ...
        for measure in range(measures):
            ...
            for beat in range(4):
                hits.append(DrumHit(
                    instrument="kick",
                    start=measure_start + float(beat),
                    velocity=105,
                    is_accent=(beat == 0)  # Slight accent on beat 1
                ))
            ...
            # Hi-hat pattern (8th notes with open hi-hat on off-beats)
            for eighth in range(8):
                pos = measure_start + (eighth * 0.5)

                # Open hi-hat on off-beats (8th note upbeats)
                if eighth % 2 == 1 and random.random() < open_hihat_density:
                    hits.append(DrumHit(
                        instrument="open_hihat",
                        start=pos,
                        velocity=75
                    ...
                    hits.append(DrumHit(
                        instrument="hihat",
                        start=pos,
                        velocity=70 if eighth % 2 == 0 else 65
                    ))

            # Eurodisco: Add 16th-note hi-hats in chorus sections
            if style == FunkStyle.EURODISCO and measure % 4 >= 2:
                for sixteenth in range(16):
                    if sixteenth % 4 != 0:  # Don't double up on 8th notes
                        pos = measure_start + (sixteenth * 0.25)
                        hits.append(DrumHit(
                            instrument="hihat",
                            start=pos,
                            velocity=50  # Quieter 16th notes
                        ))

        return hits

    def generate_disco_bass(self,
                           measures: int = 4,
                           style: FunkStyle = FunkStyle.CLASSIC_DISCO,
                           syncopation: float = 0.7) -> List[Note]:
        """
        Generate disco bass patterns

        Disco bass combines syncopated 16th-note patterns with octave jumps,
        as pioneered by Bernard Edwards (Chic). The bass creates a rhythmic
        counterpoint to the steady four-on-the-floor kick drum.
        ...
        for measure in range(measures):
            ...
            if style == FunkStyle.CLASSIC_DISCO:
                # Bernard Edwards (Chic) style: syncopated with octave jumps
                pattern = [
                    (0.0, self.key_root - 12, 0.25, 100),   # Root (strong)
                    (0.5, self.key_root - 5, 0.2, 85),      # Fifth
                    (0.75, self.key_root - 12, 0.15, 75),   # Root (syncopated)
                    (1.0, self.key_root, 0.2, 90),          # Octave jump
                    (1.5, self.key_root - 7, 0.2, 80),      # Fourth
                    (2.0, self.key_root - 12, 0.25, 95),    # Root
                    (2.5, self.key_root - 5, 0.2, 85),      # Fifth
                    (2.75, self.key_root - 10, 0.15, 75),   # b7 (approach)
                    (3.0, self.key_root - 12, 0.25, 100),   # Root
                    (3.5, self.key_root, 0.2, 90),          # Octave
                    (3.75, self.key_root - 11, 0.15, 80),   # Chromatic approach
                ]

            elif style == FunkStyle.EURODISCO:
                # Giorgio Moroder style: Sequenced, repetitive
                pattern = [
                    (0.0, self.key_root - 12, 0.25, 100),
                    (0.25, self.key_root - 12, 0.2, 80),
                    (0.5, self.key_root - 5, 0.2, 85),
                    (0.75, self.key_root - 5, 0.2, 75),
                    (1.0, self.key_root - 12, 0.25, 95),
                    (1.25, self.key_root - 12, 0.2, 80),
                    (1.5, self.key_root, 0.2, 90),
                    (1.75, self.key_root, 0.2, 80),
                    (2.0, self.key_root - 12, 0.25, 100),
                    (2.5, self.key_root - 5, 0.2, 85),
                    (3.0, self.key_root - 12, 0.25, 95),
                    (3.5, self.key_root - 7, 0.2, 85),
                ]

            else:  # NU_DISCO, ITALO_DISCO
                # Modern disco: Funk-influenced with more movement
                pattern = [
                    (0.0, self.key_root - 12, 0.25, 100),
                    (0.5, self.key_root - 5, 0.2, 85),
                    (1.0, self.key_root - 12, 0.25, 95),
                    (1.5, self.key_root, 0.2, 90),
                    (1.75, self.key_root - 10, 0.15, 75),
                    (2.0, self.key_root - 12, 0.25, 100),
                    (2.5, self.key_root - 5, 0.2, 85),
                    (2.75, self.key_root - 7, 0.15, 80),
                    (3.0, self.key_root - 12, 0.25, 95),
                    (3.5, self.key_root, 0.2, 90),
                ]

            for pos, pitch, dur, vel in pattern:
                # Skip some notes for variation
                if random.random() > syncopation and pos % 1.0 != 0.0:
                    continue
    ...
    def generate_disco_strings(self,
                               chord_progression: List[Tuple[int, str, float]],
                               style: FunkStyle = FunkStyle.CLASSIC_DISCO) -> List[Note]:
        """
        Generate disco string section arrangements

        Disco string sections feature orchestral instruments (violins, violas, cellos)
        playing both sustained pad-like chords and rhythmic stabs on off-beats.
        This approach was pioneered by arrangers like Gene Page (for Donna Summer)
        and brought lush orchestration to dance music.
        ...
        # String ranges (MIDI)
        VIOLIN_RANGE = (55, 96)    # G3-C7
        VIOLA_RANGE = (48, 84)     # C3-C6
        CELLO_RANGE = (36, 72)     # C2-C5

        for root, quality, duration in chord_progression:
            # Build string voicing (wide spacing)
            if quality == '7':
                chord_tones = [root, root + 4, root + 7, root + 10]
            elif quality == 'minor7':
                chord_tones = [root, root + 3, root + 7, root + 10]
            ...
            # Sustained pad (whole duration)
            string_voicing = [
                chord_tones[0] - 12,        # Cello (root, low)
                chord_tones[1],             # Viola (3rd)
                chord_tones[2] + 12,        # Violin II (5th, high)
                chord_tones[0] + 24,        # Violin I (root, octave up)
            ]

            for pitch in string_voicing:
                notes.append(Note(
                    pitch=pitch,
                    start=current_time,
                    duration=duration,
            ...
            stab_positions = [
                (0.5, 0.3, 85),    # Off-beat
                (1.5, 0.3, 85),    # Off-beat
                (2.5, 0.3, 90),    # Off-beat (stronger)
                (3.5, 0.3, 85),    # Off-beat
            ]

            for pos, dur, vel in stab_positions:
                # Skip some stabs for variation
                if random.random() > 0.7:
                    continue

                stab_time = current_time + pos

                # Stabs in close harmony
                for pitch in string_voicing[1:]:  # Skip bass note
                    notes.append(Note(
                        pitch=pitch,
                        start=stab_time,
                        duration=dur,
    ...
    def generate_disco_guitar(self,
                             measures: int = 4,
                             style: FunkStyle = FunkStyle.CLASSIC_DISCO) -> List[Note]:
        """
        Generate disco guitar patterns

        Nile Rodgers (Chic) pioneered the disco guitar technique: tight, muted
        16th-note strumming creating a percussive, rhythmic texture. As Rodgers
        explains: "I'm really a drummer who happens to play guitar."
        ...
        for measure in range(measures):
            ...
            # Nile Rodgers style: Muted 16th-note strumming
            # Pattern emphasizes off-beats and syncopation
            for sixteenth in range(16):
                pos = measure_start + (sixteenth * 0.25)

                # Determine if this is an accented hit
                # Accents on: 2-and, 3-e-and-a, 4-and
                is_accent = sixteenth in [5, 9, 10, 11, 13]

                # Skip some hits for rhythmic variation
                if not is_accent and random.random() > 0.6:
                    continue

                velocity = 85 if is_accent else 60
                duration = 0.12 if is_accent else 0.08  # Very short (muted)

                # Three-note chord voicing
                chord_notes = [
                    self.key_root + 7,   # Fifth
                    self.key_root + 12,  # Octave
                    self.key_root + 16,  # Third (octave up)
                ]
    ...
    def generate_eurodisco_synth(self,
                                measures: int = 4,
                                arpeggio_speed: float = 16.0) -> List[Note]:
        """
        Generate Eurodisco synthesizer arpeggios

        Giorgio Moroder's "I Feel Love" (1977) revolutionized disco with its
        entirely electronic production featuring sequenced 16th-note synth arpeggios.
        This became the blueprint for Eurodisco and later electronic dance music.

        Args:
            measures: Number of measures
            arpeggio_speed: Notes per measure (8=8th notes, 16=16th notes)

        Returns:
            List of synthesizer arpeggio notes
        """
        notes = []
        notes_per_beat = arpeggio_speed / 4.0
        step_duration = 1.0 / notes_per_beat

        # Arpeggio pattern (based on chord tones)
        # Pattern: Root-Fifth-Octave-Fifth (ascending/descending)
        arpeggio_pattern = [
            self.key_root,           # Root
            self.key_root + 7,       # Fifth
            self.key_root + 12,      # Octave
        ...
        for measure in range(measures):
            ...
            step_count = int(4.0 / step_duration)  # Total steps in measure

            for step in range(step_count):
                pos = measure_start + (step * step_duration)

                # Cycle through arpeggio pattern
                note_index = step % len(arpeggio_pattern)
                pitch = arpeggio_pattern[note_index]

                # Velocity variation for groove
                velocity = 90 if step % 4 == 0 else 75

                notes.append(Note(
                    pitch=pitch,
                    start=pos,
                    duration=step_duration * 0.9,  # Slight gap between notes
                    velocity=velocity,
                    articulation="normal"
                ))
    ...
    def generate_disco_groove(self,
                             measures: int = 8,
                             style: FunkStyle = FunkStyle.CLASSIC_DISCO) -> FunkGroove:
        """
        Generate complete disco groove with all instruments

        Disco combines the rhythmic foundation of funk with orchestral arrangements
        and electronic production. The four-on-the-floor kick provides steady drive
        while syncopated bass, rhythmic guitar, and lush strings create the
        signature disco sound.

        Args:
            measures: Number of measures to generate (typically 8 or 16)
            style: Disco style (Classic, Eurodisco, Italo, Nu-Disco)

        Returns:
            FunkGroove with drums, bass, guitar, strings, and synths
        """
        # Generate chord progression for disco
        chord_progression = self._generate_disco_chord_progression(measures, style)

        # Drums: Four-on-the-floor
        drums = self.generate_disco_drums(
            measures=measures,
            style=style,
            open_hihat_density=0.8
        )

        # Bass: Syncopated disco bass
        bass = self.generate_disco_bass(
            measures=measures,
            style=style,
            syncopation=0.75
        )

        # Guitar: Nile Rodgers-style rhythmic guitar
        guitar = self.generate_disco_guitar(
            measures=measures,
            style=style
        )

        # Strings: Orchestral string section
        strings = self.generate_disco_strings(
            chord_progression=chord_progression,
            style=style
        )

        # Keys: Rhodes or synth depending on style
        if style == FunkStyle.EURODISCO or style == FunkStyle.ITALO_DISCO:
            # Eurodisco: Synth arpeggios
            keys = self.generate_eurodisco_synth(
                measures=measures,
                arpeggio_speed=16.0
            )
        else:
            # Classic/Nu-Disco: Rhodes comping
            keys = self.generate_rhodes_comp(
                chord_progression=chord_progression,
                voicing="add9"
            )

        # Horns: Add for classic disco (Tower of Power influence)
        horns = []
        if style == FunkStyle.CLASSIC_DISCO:
            horns = self.generate_horn_section(
                chord_progression=chord_progression,
        ...
        # Combine strings and horns
        combined_horns = strings + horns
        ...
        return FunkGroove(
            ...
            guitar=guitar,
            keys=keys,
            horns=combined_horns,  # Strings + horns
            tempo=self.tempo,
            measures=measures
        )

    def _generate_disco_chord_progression(self,
                                          measures: int,
                                          style: FunkStyle) -> List[Tuple[int, str, float]]:
        """
        Generate disco chord progression

        Disco often uses simple, repeating chord progressions derived from funk
        and R&B. Classic progressions include I-IV-V and vi-IV-I-V patterns.
        ...
        if style == FunkStyle.CLASSIC_DISCO:
            # Classic disco: I-IV-V-IV (4-bar loop)
            disco_prog = [
                (self.key_root, 'maj7', 4.0),       # I
                (self.key_root + 5, 'maj7', 4.0),   # IV
                (self.key_root + 7, '7', 4.0),      # V7
                (self.key_root + 5, 'maj7', 4.0),   # IV
            ]
            for i in range(measures // 4):
                progression.extend(disco_prog)

        elif style == FunkStyle.EURODISCO or style == FunkStyle.ITALO_DISCO:
            # Eurodisco: Simple I-IV vamp (emphasizes synth arpeggios)
            for i in range(measures):
                if i % 2 == 0:
                    progression.append((self.key_root, 'maj7', 4.0))
                else:
                    progression.append((self.key_root + 5, 'maj7', 4.0))

        else:  # NU_DISCO
            # Nu-disco: vi-IV-I-V (modern pop progression)
            nu_disco_prog = [
                (self.key_root + 9, 'minor7', 4.0),  # vi
                (self.key_root + 5, 'maj7', 4.0),    # IV
                (self.key_root, 'maj7', 4.0),        # I
                (self.key_root + 7, '7', 4.0),       # V7
            ]
            for i in range(measures // 4):
                progression.extend(nu_disco_prog)
...
if __name__ == "__main__":
    ...
    # ========================================================================
    # DISCO TESTS (Tests 28-33)
    # ========================================================================

    # Test 28: Test disco four-on-the-floor drums
    print("\n[Test 28] Testing disco four-on-the-floor drums...")
    disco_drums = generator.generate_disco_drums(
        measures=2,
        style=FunkStyle.CLASSIC_DISCO,
        open_hihat_density=0.8
    )
    assert len(disco_drums) > 0
    # Verify four-on-the-floor (kick on every quarter note)
    kick_hits = [h for h in disco_drums if h.instrument == "kick"]
    assert len(kick_hits) == 8  # 4 kicks per measure * 2 measures
    # Verify kicks are on beats 1, 2, 3, 4
    kick_positions = [h.start % 4.0 for h in kick_hits[:4]]
    assert kick_positions == [0.0, 1.0, 2.0, 3.0]
    print(f"✓ Generated {len(disco_drums)} disco drum hits with four-on-the-floor")

    # Test 29: Test disco bass (Bernard Edwards style)
    print("\n[Test 29] Testing disco bass (Bernard Edwards/Chic style)...")
    disco_bass = generator.generate_disco_bass(
        measures=2,
        style=FunkStyle.CLASSIC_DISCO,
        syncopation=0.8
    )
    assert len(disco_bass) > 0
    # Verify staccato articulation
    staccato_count = sum(1 for n in disco_bass if n.articulation == "staccato")
    assert staccato_count > 0
    print(f"✓ Generated {len(disco_bass)} disco bass notes")

    # Test 30: Test disco string sections
    print("\n[Test 30] Testing disco string sections...")
    disco_chord_prog = [
        (60, 'maj7', 4.0),
        (65, 'maj7', 4.0),
    ]
    disco_strings = generator.generate_disco_strings(
        chord_progression=disco_chord_prog,
        style=FunkStyle.CLASSIC_DISCO
    )
    assert len(disco_strings) > 0
    # Verify both sustained and staccato notes
    sustained_count = sum(1 for n in disco_strings if n.articulation == "sustained")
    staccato_count = sum(1 for n in disco_strings if n.articulation == "staccato")
    assert sustained_count > 0
    assert staccato_count >= 0  # May have staccato stabs
    print(f"✓ Generated {len(disco_strings)} string notes ({sustained_count} sustained, {staccato_count} staccato)")

    # Test 31: Test disco guitar (Nile Rodgers style)
    print("\n[Test 31] Testing disco guitar (Nile Rodgers/Chic style)...")
    disco_guitar = generator.generate_disco_guitar(
        measures=2,
        style=FunkStyle.CLASSIC_DISCO
    )
    assert len(disco_guitar) > 0
    # Verify short staccato notes (muted strumming)
    short_notes = sum(1 for n in disco_guitar if n.duration < 0.15)
    assert short_notes > len(disco_guitar) * 0.5  # At least 50% should be short
    print(f"✓ Generated {len(disco_guitar)} disco guitar notes")

    # Test 32: Test Eurodisco synth arpeggios (Giorgio Moroder)
    print("\n[Test 32] Testing Eurodisco synth arpeggios (Giorgio Moroder style)...")
    eurodisco_synth = generator.generate_eurodisco_synth(
        measures=2,
        arpeggio_speed=16.0
    )
    assert len(eurodisco_synth) > 0
    # Should have 16 notes per measure (16th notes)
    assert len(eurodisco_synth) >= 30  # At least ~32 notes (allow for some variation)
    print(f"✓ Generated {len(eurodisco_synth)} Eurodisco synth notes")

    # Test 33: Test complete disco grooves (all styles)
    print("\n[Test 33] Testing complete disco grooves (all styles)...")

    # Classic Disco
    classic_disco = generator.generate_disco_groove(
        measures=4,
        style=FunkStyle.CLASSIC_DISCO
    )
    assert classic_disco.measures == 4
    assert len(classic_disco.drums) > 0
    assert len(classic_disco.bass) > 0
    assert len(classic_disco.guitar) > 0
    assert len(classic_disco.keys) > 0
    assert len(classic_disco.horns) > 0  # Should include strings + horns
    print(f"✓ Classic Disco: {len(classic_disco.drums)} drums, {len(classic_disco.bass)} bass, {len(classic_disco.horns)} strings/horns")

    # Eurodisco
    eurodisco = generator.generate_disco_groove(
        measures=4,
        style=FunkStyle.EURODISCO
    )
    assert eurodisco.measures == 4
    assert len(eurodisco.keys) > 0  # Should have synth arpeggios
    print(f"✓ Eurodisco: {len(eurodisco.keys)} synth arpeggio notes")

    # Nu-Disco
    nu_disco = generator.generate_disco_groove(
        measures=4,
        style=FunkStyle.NU_DISCO
    )
    assert nu_disco.measures == 4
    print(f"✓ Nu-Disco: {len(nu_disco.drums)} drums, {len(nu_disco.bass)} bass")

    # Test 34: Test disco via generate_funk_groove
    print("\n[Test 34] Testing disco via generate_funk_groove delegation...")
    disco_via_funk = generator.generate_funk_groove(
        measures=4,
        style=FunkStyle.CLASSIC_DISCO
    )
    assert disco_via_funk.measures == 4
    assert len(disco_via_funk.drums) > 0
    # Verify four-on-the-floor
    kick_hits = [h for h in disco_via_funk.drums if h.instrument == "kick"]
    assert len(kick_hits) == 16  # 4 per measure * 4 measures
    print(f"✓ Disco delegation working correctly")

    print("\n" + "=" * 70)
    print("✅ ALL 34 TESTS PASSED! (27 Funk + 7 Disco)")
    print("=" * 70)
    print("\n🎵 Funk, Soul & Disco Generator is ready to groove! 🎵")
    ...
    print("  ✓ Disco four-on-the-floor (Chic, Donna Summer)")
    print("  ✓ Bernard Edwards disco bass")
    print("  ✓ Nile Rodgers rhythmic guitar")
    print("  ✓ Disco orchestral string sections")
    print("  ✓ Giorgio Moroder Eurodisco synth arpeggios")
