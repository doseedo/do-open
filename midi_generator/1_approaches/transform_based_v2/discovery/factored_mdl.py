...
class FactoredObject:
    ...
    New factorization (v2):
    - rhythm: Binary onset pattern (unchanged)
    - pitch_class: Mod 12 of MIDI pitch (0-11)
    - octave: Floor division of MIDI pitch (0-9)
    - duration: Note lengths (unchanged)
    - velocity: Quantized to 8 levels (0-7)
    - contour: COMPUTED from pitch_class + octave, not stored
    """
    # Identity
    piece_id: str
    track_id: int
    ...
    # Factored components (v2)
    rhythm: np.ndarray          # (T,) binary onset pattern
    pitch_class: np.ndarray     # (N,) pitch class 0-11 (pitch % 12)
    octave: np.ndarray          # (N,) octave 0-9 (pitch // 12)
    velocities: np.ndarray      # (N,) velocity quantized to 8 levels (0-7)
    ...
    def pitches(self) -> np.ndarray:
        """Reconstruct absolute MIDI pitches from pitch_class and octave."""
        return self.pitch_class + self.octave * 12

    @property
    def pitch_contour(self) -> np.ndarray:
        """Compute contour (intervals) from pitch sequence. COMPUTED, not stored."""
        pitches = self.pitches
        if len(pitches) > 1:
            return np.diff(pitches).astype(np.int32)
        return np.array([], dtype=np.int32)

    def get_contour(self) -> np.ndarray:
        """Compute contour from pitch sequence. Explicit method for clarity."""
    ...
    def pitch_class_hash(self) -> int:
        """Hash for fast pitch-class sequence comparison."""
    ...
    def contour_hash(self) -> int:
        """Hash for fast contour comparison. Computed from pitch sequence."""
...
def factor_tensor(tensor: np.ndarray, piece_id: str, track_id: int,
    ...
    if not note_events:
        # Empty object - v2 format with pitch_class, octave, quantized velocity
        return FactoredObject(
            ...
            scale=scale,
            rhythm=rhythm,
            pitch_class=np.array([], dtype=np.int32),
            octave=np.array([], dtype=np.int32),
            velocities=np.array([], dtype=np.int32),
    ...
    velocities_float = np.array([e[2] for e in note_events], dtype=np.float32)
    durations = np.array([e[3] for e in note_events], dtype=np.int32)

    # V2: Factor pitches into pitch_class and octave
    pitch_class = (pitches % 12).astype(np.int32)
    octave = (pitches // 12).astype(np.int32)

    # V2: Quantize velocities to 8 levels (0-7)
    # velocity range 0.0-1.0 -> 0-7
    velocities = np.clip((velocities_float * 8).astype(np.int32), 0, 7)
...
def match_velocities(source: np.ndarray, target: np.ndarray,
                     max_error: float = 1.5) -> Tuple[bool, Optional[ComponentTransform], float]:
    """
    Check if source velocities match target, possibly scaled.

    V2: Velocities are now quantized to 8 levels (0-7), so max_error is adjusted.
    """
    if len(source) != len(target) or len(source) == 0:
        return False, None, float('inf')

    # Exact match (allow 1-level difference per note on average)
    error = np.mean(np.abs(source - target))
    if error < max_error:
        return True, None, error
    ...
    if source_mean > 0.1:  # Avoid division by near-zero (adjusted for 0-7 range)
        scale = target_mean / source_mean

        # Only consider reasonable scales
        if 0.3 <= scale <= 3.0:
            scaled = np.clip(source * scale, 0, 7)  # V2: clip to 0-7 range
...
def compute_component_compression(
    ...
    if component == 'rhythm':
        ...
        # Intervals typically -24 to +24, need ~6 bits (computed from pitches)
        components = [o.pitch_contour for o in valid_objects]
        bits_per_value = 6.0
        total_values = sum(len(c) for c in components)
    elif component == 'pitches':
        # MIDI pitches 0-127, need 7 bits (computed from pitch_class + octave)
        components = [o.pitches for o in valid_objects]
        bits_per_value = 7.0
        total_values = sum(len(c) for c in components)
    elif component == 'pitch_class':
        # V2: Pitch class 0-11, need 4 bits
        components = [o.pitch_class for o in valid_objects]
        bits_per_value = 4.0
        total_values = sum(len(c) for c in components)
    elif component == 'octave':
        # V2: Octave 0-9, need 4 bits
        components = [o.octave for o in valid_objects]
        bits_per_value = 4.0
        total_values = sum(len(c) for c in components)
    elif component == 'velocities':
        # V2: Velocities quantized to 8 levels (0-7), need 3 bits
...
def compute_full_compression_stats(
    ...
    # V2: Use pitch_class and octave instead of pitches, keep contour for compatibility
    for component in ['rhythm', 'pitch_class', 'octave', 'velocities', 'durations']:
    ...
    # Also compute contour stats for reference (it's derived from pitch_class + octave)
    contour_cs = compute_component_compression(objects, 'contour', verbose)
    stats['contour'] = contour_cs
