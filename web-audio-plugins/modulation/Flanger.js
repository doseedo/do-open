/**
 * Flanger Effect Plugin (AudioWorklet Version)
...
class FlangerPlugin {
  constructor(audioContext, options = {}) {
    ...
    // Parameters
    this._params = {
      rate: 0.3,        // LFO speed in Hz (0.01 to 10)
      depth: 100,       // Modulation intensity (0-100%)
      feedback: 50,     // Feedback amount (0-100%)
      delay: 2,         // Base delay time in ms (1-10)
  ...
  async setupAudioWorklet() {
    try {
      ...
      const workletPath = new URL('./worklets/flanger-processor.js', import.meta.url).href;
      await this.context.audioWorklet.addModule(workletPath);

      // Create the AudioWorklet node
      this.workletNode = new AudioWorkletNode(this.context, 'flanger-processor', {
        numberOfInputs: 1,
        numberOfOutputs: 1,
      ...
      console.error('Failed to setup AudioWorklet for Flanger:', error);
  ...
  /**
   * Set base delay time
   * @param {number} ms - Delay time in milliseconds (1 to 10)
   */
  setDelayTime(ms) {
    this._params.delay = Math.max(1, Math.min(10, ms));
    this.updateWorkletParams();
  }
