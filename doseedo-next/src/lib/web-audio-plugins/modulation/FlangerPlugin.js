/**
 * FlangerPlugin - Professional Flanger Effect
 *
 * AudioWorklet-based flanger with:
 * - Short delay with LFO modulation (jet-plane whoosh)
 * - Positive and negative feedback
 * - Manual delay offset control
...
export class FlangerPlugin extends BasePlugin {
  constructor(audioContext, options = {}) {
    super(audioContext, {
      category: 'modulation',
      description: 'Classic flanger effect with feedback and manual control',
      name: 'Flanger',
      ...options
    });
  ...
  async initialize() {
    ...
    try {
      ...
      await this.audioContext.audioWorklet.addModule(
        new URL('../worklets/flanger-processor.js', import.meta.url).href
      );
      ...
      this.registerParameter('manual', this.workletNode.parameters.get('manual'), {
        min: 0,
        max: 100,
        default: this.defaults.manual,
        unit: '%',
        label: 'Manual',
        type: 'continuous'
      });
      ...
      this.setParameter('manual', this.defaults.manual);
      ...
      console.error('Failed to initialize FlangerPlugin:', error);
      throw error;
    }
  }
  ...
  async processOffline(inputBuffer) {
    ...
    const workletNode = new AudioWorkletNode(offlineContext, 'flanger-processor');
    ...
    workletNode.parameters.get('manual').value = this.getParameter('manual').param.value;
