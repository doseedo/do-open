...
function ThemeEditor() {
  ...
  const glassControls = {
    ...
    glassPanels: [
      { name: 'Panel Opacity', var: '--glass-panel-opacity', type: 'range', min: 0.1, max: 0.9, step: 0.05 },
      { name: 'Panel Blur', var: '--glass-panel-blur', type: 'range', min: 0, max: 30, step: 1 },
    ]
  };
    ...
    <div className="theme-editor">
      ...
      {isGlassMode && (
        <div className="theme-editor-section glass-controls">
          ...
          <div className="glass-control-group">
            <h5>Panel Backgrounds (Sidebars, DAW)</h5>
            {glassControls.glassPanels.map(control => (
              <SliderInput
                key={`${control.var}-${refreshKey}`}
                label={control.name}
                varName={control.var}
...
function SliderInput({ label, varName, min, max, step, unit = '', onChange }) {
  ...
  const getDefaultValue = () => {
    // Panel settings
    if (varName.includes('panel-opacity')) return 0.5;
    if (varName.includes('panel-blur')) return 16;
    // Standard glass settings
