...
function Navbar() {
  ...
  return (
    <div id="navbar">
      ...
      <button
        className={`nav-btn ${state.cinemaMode ? 'active' : ''}`}
        title="Toggle Cinema Mode"
        onClick={() => dispatch({ type: 'TOGGLE_CINEMA_MODE' })}
        style={{
          backgroundColor: state.cinemaMode ? 'rgba(102, 126, 234, 0.3)' : 'transparent',
          border: state.cinemaMode ? '1px solid rgba(102, 126, 234, 0.5)' : '1px solid transparent'
        }}
      >
