...
const VideoUploadOptimized = React.memo(() => {
  ...
  // Use shared state from context for video persistence across cinema mode
  const videoPreviewUrl = state.video?.videoPreviewUrl;
  const showVideo = !!videoPreviewUrl;

  // Ref for video element to control playback
  const videoRef = useRef(null);
  const isSeekingRef = useRef(false);
  const frameCanvasRef = useRef(null);
  const ambientGlowRef = useRef(null); // For ambient glow cleanup
  ...
  const processVideoFile = useCallback(async (file) => {
    ...
    // Store video info in context (shared state for cinema mode)
    dispatch({
      type: 'SET_VIDEO_INFO',
      payload: {
        videoFile: file,
    ...
    try {
      ...
      // Don't show alert for server connection errors
    }
  }, [processVideo, useWhisper, dispatch]);
  ...
  // Initialize ambient glow effect for video
  useEffect(() => {
    const video = videoRef.current;
    if (!video || !showVideo || !videoPreviewUrl) return;

    // Wait for video to be ready
    const initAmbientGlow = () => {
      // Clean up previous instance
      if (ambientGlowRef.current) {
        ambientGlowRef.current.destroy();
        ambientGlowRef.current = null;
      }

      try {
        ambientGlowRef.current = new AmbientGlow(video, {
          blur: 80,           // Blur radius for the glow
          opacity: 0.65,      // Opacity of the glow effect
          brightness: 1.2,    // Brightness multiplier
          saturate: 1.3       // Color saturation
        });
        console.log('✨ Ambient glow effect initialized');
      } catch (err) {
        console.warn('⚠️ Failed to initialize ambient glow:', err);
      }
    };
    ...
    if (video.readyState >= 1) {
      ...
      video.addEventListener('loadedmetadata', initAmbientGlow, { once: true });
    }
    ...
    return () => {
      if (ambientGlowRef.current) {
        ambientGlowRef.current.destroy();
        ambientGlowRef.current = null;
        console.log('🧹 Ambient glow effect cleaned up');
      }
    };
    ...
    <div className={styles.uploadGrid}>
      ...
      {(showVideo || isProcessing) && (
        <div className={styles.videoContainer}>
          ...
          {/* Processing Error - hidden to avoid popup when servers are down */}
          {/* processingError && (
