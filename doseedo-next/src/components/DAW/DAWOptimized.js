...
const DAWOptimized = React.memo(({ maxTracksHeight = 600, panelWidth = 400, pluginMode = false }) => {
  ...
  // Track actual sidebar width to know when to drop BPM/Meter labels
  const [busLabelWidth, setBusLabelWidth] = React.useState(panelWidth);
  React.useEffect(() => {
  ...
  // Hide BPM/Meter labels when tight; inputs always visible
  const showTempoLabels = busLabelWidth >= 400;
    ...
    <div ref={dawGridRef} className={`${styles.dawGrid} ${pluginMode ? styles.pluginMode : ''}`}>
      ...
      >
        ...
        {!pluginMode && (
          ...
          <div className={styles.timelineSpacer1} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 8px 4px 8px', paddingRight: '18px' }}>
            {/* Tempo + Meter Group (LEFT) — fixed size, never shrinks */}
            <div style={{
              display: 'flex',
              alignItems: 'center',
              gap: '6px',
            ...
            {/* Spacer — absorbs free space between left and right groups */}
            <div style={{ flex: '1 1 0' }} />

            {/* Automation + Transport Group (RIGHT) — fixed size, never shrinks */}
