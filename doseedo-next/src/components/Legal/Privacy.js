...
const Privacy = () => (
  <div className={styles.legal}>
    <PageTopbar section="More" title="Privacy" meta="last updated 2026-04-25" />
    ...
    <div className={styles.content}>
      ...
      <div className={styles.section}>
        ...
        <p className={styles.sectionText} style={{ marginTop: 12 }}>
          <strong>Embedded watermark.</strong> Audio generated through the Service carries an inaudible watermark (AudioSeal) embedded at the source's native sample rate. The watermark conveys a "doseedo signal" classifier value, not a unique identifier; uniqueness comes from the SHA-256 of the audio bytes the user receives, which is what binds to the on-chain record described below. The watermark is designed to survive normal post-production (MP3/Opus encoding, EQ, time-stretch, analog re-recording) and is imperceptible at typical listening conditions. Free-tier exports are always watermarked. Pro, Studio, and Power exports default to watermarked but can be exported clean on a per-generation basis. The watermark itself contains no personally identifying information.
        </p>
        <p className={styles.sectionText} style={{ marginTop: 12 }}>
          <strong>On-chain attestation registry.</strong> At generation time, a canonical record (RFC-8785 / JCS) containing the SHA-256 of the audio, the generation ID, tier-at-gen-time, model version, and a UTC timestamp is built and its keccak256 is submitted to a public registry contract on the Polygon network. The on-chain footprint is one bytes32 plus a short metadata URI pointing back to the Doseedo verifier — no audio, no personal information, no user identifier. The off-chain database row holds the rest of the metadata under the same controls as Your Content.
        </p>
        <p className={styles.sectionText} style={{ marginTop: 12 }}>
        ...
        <p className={styles.sectionText} style={{ marginTop: 12 }}>
          <strong>Opting out of the watermark.</strong> Pro, Studio, and Power users may export specific generations without the audio-side watermark using the per-export toggle in the Service. The on-chain attestation is still created — it is bound to the SHA-256 of the audio bytes you receive, not to the watermark — but the exported audio will not carry the inaudible signal. Free-tier exports are always watermarked.
        </p>
      </div>
