    ...
    try:
        ...
        if ACE_STEP_PIPELINE is not None:
            ...
            if detailed_mode and lyrics and ref_audio_path:
                print(f"\n{'='*80}")
                print(f"🎨 DETAILED MODE ENABLED - Phrase-by-Phrase Generation")
                print(f"{'='*80}")
                print(f"   Using MFA (Montreal Forced Aligner) for phrase segmentation")
                print(f"   Noise-to-noise architecture with noise_level={noise_level}")
                print(f"{'='*80}\n")

                try:
                    # Import detailed mode generator
                    from core.generate_ace_step_detailed import generate_detailed_mode

                    # Run detailed mode generation
                    result = generate_detailed_mode(
                        pipeline=ACE_STEP_PIPELINE,
                        lyrics=lyrics,
                    ...
                    if result.get('output_path'):
                        ...
                        print(f"   Phrases generated: {len(result.get('phrase_paths', []))}")
                    else:
                        raise Exception("Detailed mode did not return output_path")

                except Exception as e:
                    print(f"⚠️  Detailed mode failed: {e}")
...
async def generate_ace_step_endpoint(
    ...
    Parameters:
        ...
        detailed_mode: Enable phrase-by-phrase generation with MFA (default: False)
    """
    import subprocess
    import uuid
    from pathlib import Path
...
async def get_ace_step_task_status(task_id: str):
    ...
    if task.state == "SUCCESS":
        result = task.result
        print(f"📊 ACE-Step Task result type: {type(result)}, value: {result}")
        ...
        print(f"📊 ACE-Step Progress: {num_completed}/{total_voices} voices completed ({progress*100:.1f}%)")
        print(f"📊 Completed voices: {completed_voices}")
        print(f"📦 Input files available: {len(input_files)} voices")
...
async def download_ace_step_audio(process_id: str, filename: str):
    ...
    NOTE: This endpoint is deprecated - new generations return GCS URLs directly.
    This is kept for backward compatibility with old file_paths.
    """
    ...
    # Try GCS (in case file was uploaded and local copy deleted)
    try:
        from gcs_storage import gcs_file_exists, download_from_gcs, get_gcs_path
        import tempfile
