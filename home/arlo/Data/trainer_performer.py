import sys
# Add dø first for DoTrainComponents (custom model), then ACE-Step for schedulers
...
import argparse, os, json, math, torch, torch.nn.functional as F
...
#TRAIN FROM SCRATCH
    ...
    def _setup_class_weights(self, manifest_json):
        ...
        try:
            ...
            alpha = 0.6  # 0=no weight, 1=full inverse frequency
            ...
            # Ensure weights are valid (no zeros that would zero out loss)
            self.group_ce_w = self.group_ce_w.clamp_min(1e-6)
            self.sub_ce_w = self.sub_ce_w.clamp_min(1e-6)
    ...
    def on_after_backward(self) -> None:
        ...
        if getattr(self, "use_ctrl_branch", False):
            self._ctrl_residuals = None
        
        # Gradient flow monitoring (every 100 steps to avoid log spam)
        if (self.global_step % 100) == 0:
            # Handle different transformer architectures safely
            try:
                if hasattr(self.transformers, 'blocks') and len(self.transformers.blocks) > 0:
                    self.log("gn/transformer_last", self._module_grad_norm(self.transformers.blocks[-1]), on_step=True)
                elif hasattr(self.transformers, 'transformer_blocks') and len(self.transformers.transformer_blocks) > 0:
                    self.log("gn/transformer_last", self._module_grad_norm(self.transformers.transformer_blocks[-1]), on_step=True)
                else:
                    # Fallback to entire transformer if no blocks found
                    self.log("gn/transformer", self._module_grad_norm(self.transformers), on_step=True)
            except Exception:
                # Skip transformer monitoring if it fails
                pass
            
            try:
                self.log("gn/cond_adapter", self._module_grad_norm(self.cond_adapter), on_step=True)
                self.log("gn/ctrl_enc", self._module_grad_norm(self.ctrl_enc), on_step=True)
                if hasattr(self, "ctrlnet"):
                    self.log("gn/ctrlnet", self._module_grad_norm(self.ctrlnet), on_step=True)
                self.log("gn/group_head", self._module_grad_norm(self.group_head), on_step=True)
                self.log("gn/sub_head", self._module_grad_norm(self.sub_head), on_step=True)
            except Exception:
                # Skip if any gradient monitoring fails
                pass
    ...
    def _recon_w(self) -> float:
        # Cosine decay from 0.35 → 0.10 over 60k steps for recon weight scheduling
        T = 60000.0
        t = min(float(self.global_step), T)
        w0, w1 = 0.35, 0.10
        cos = 0.5 * (1.0 + math.cos(math.pi * t / T))
        return w1 + (w0 - w1) * cos

    def _module_grad_norm(self, module) -> float:
        # Compute gradient norm for a module without log spam
        sq = 0.0
        for p in module.parameters():
            if p.grad is not None:
                sq += p.grad.data.float().pow(2).sum().item()
        return (sq ** 0.5)
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        # Ensure _ctrl_residuals is properly initialized for backward compatibility
        if not hasattr(self, '_ctrl_residuals'):
            if self.use_ctrl_branch and hasattr(self, '_ctrl_handles'):
                n_inject = len(self._ctrl_handles)
                self._ctrl_residuals = [None] * n_inject
            else:
                self._ctrl_residuals = None
...
# OG ACE STEP DIFFUSION
    ...
    def training_step(self, batch, batch_idx):
        ...
        gw = self.group_ce_w.to(group_logits.device, dtype=group_logits.dtype)
        sw = self.sub_ce_w.to(sub_logits.device, dtype=sub_logits.dtype)
        aux_loss = F.cross_entropy(group_logits, group_tgt, weight=gw) \
                 + F.cross_entropy(sub_logits, sub_tgt, weight=sw)
        ...
        # adapter (randomized gain) - reduce variance early in training
        import random
        base_gain = self._adapter_gain_scale()
        if self.global_step < 5000:
            random_multiplier = random.uniform(0.9, 1.1)  # less variance early on
        else:
            random_multiplier = random.uniform(0.8, 1.2)  # full variance later
        ...
        # no-x-attn forward (use continuous tau for proper timestep ↔ noise coupling)
        v_pred = self._call_transformer_no_xattn(latents=x_in, t=tau_f32)
        ...
        # recon weighting by activity - weight at time dimension before reducing
        err_bt = (x0_hat - x0).pow(2).mean(dim=(1,2))     # [B, T]  (mean over C,H)
        time_w = 1.0 + 0.5 * (pr_tgt.amax(dim=1) > 0).to(x0_hat.dtype)  # [B, T]
        recon_per_ex = (err_bt * time_w).mean(dim=1)      # [B]
        ...
        v_target = z - x0
        v_loss   = F.mse_loss(v_pred, v_target)  # primary loss for v-prediction

        # Log v_loss per tau-bin occasionally for debugging uniform learning
        if self.global_step % 500 == 0:
            with torch.no_grad():
                tau_bins = [(0.0, 0.25), (0.25, 0.5), (0.5, 0.75), (0.75, 1.0)]
                for i, (tau_min, tau_max) in enumerate(tau_bins):
                    mask = (tau_f32 >= tau_min) & (tau_f32 < tau_max)
                    if mask.any():
                        v_loss_bin = F.mse_loss(v_pred[mask], v_target[mask])
                        self.log(f"dbg/v_loss_bin_{i}", v_loss_bin, on_step=True)

        # v_loss is now primary, recon_loss becomes a scheduled regularizer
        w_recon = self._recon_w()
        loss = v_loss + w_recon * recon_loss + self.adapter_l2 * cond_reg + aux_w * aux_loss + self.pr_loss_weight * pr_loss
        self.log("reg/recon_w", torch.tensor(w_recon, device=loss.device), on_step=True)
    ...
    def validation_step(self, batch, batch_idx):
        # Mirror training objective but disable all train-time augmentations
        def to_device(x, d):
            if isinstance(x, torch.Tensor): return x.to(d)
            if isinstance(x, dict): return {k: to_device(v,d) for k,v in x.items()}
            if isinstance(x, list): return [to_device(i,d) for i in x]
        ...
        # NO augmentations in validation - use clean encodec tokens
        enc_tok = batch["encodec_tokens"]  # no dropout, no swapping

        tokens, mask = self.ctrl_enc(
            piano_roll=batch["conds"]["piano_roll"],  # no cfg drop, no partial masking
            amp=batch["conds"]["amp"],
            rframe=batch["conds"]["rframe"],
            rbend=batch["conds"]["rbend"],
            rbend_mask=batch["conds"]["rbend_mask"],
        ...
        # Recompute control residuals for validation parity with training
        if self.use_ctrl_branch:
            pr_128 = batch["conds"]["piano_roll"].to(x0.device, dtype=x0.dtype)
            amp_1t = batch["conds"]["amp"].to(x0.device, dtype=x0.dtype).unsqueeze(1)
            if amp_1t.shape[-1] != pr_128.shape[-1]:
                amp_1t = F.interpolate(amp_1t, size=pr_128.shape[-1], mode="nearest")
            ctrl_in = torch.cat([pr_128, amp_1t], dim=1)
            res_list = self.ctrlnet(ctrl_in, T_out_list=[x0.shape[-1]] * len(self.ctrlnet.to_blocks),
                                    instrument_token=tokens[:,0,:])
            scale = float(getattr(self.hparams, "control_scale", 1.0))
            self._ctrl_residuals = [r * scale for r in res_list]

        # Aux classification (same as training)
        inst_tok = tokens[:, 0, :]
        group_logits = self.group_head(inst_tok)
        sub_logits = self.sub_head(inst_tok)
        group_tgt = batch["instrument"]["group_id"].long().to(group_logits.device)
        ...
        gw = self.group_ce_w.to(group_logits.device, dtype=group_logits.dtype)
        sw = self.sub_ce_w.to(sub_logits.device, dtype=sub_logits.dtype)
        aux_loss = F.cross_entropy(group_logits, group_tgt, weight=gw) + F.cross_entropy(sub_logits, sub_tgt, weight=sw)

        # Same v-prediction objective as training (no randomness in tau)
        tau_f32 = torch.rand(B, device=x0.device, dtype=torch.float32).clamp_(1e-4, 1 - 1e-4)
        sigma = tau_f32.to(x0.dtype).view(B, *([1] * (x0.ndim - 1)))
        z = torch.randn_like(x0)
        x_noisy = (1.0 - sigma) * x0 + sigma * z

        # NO adapter randomness in validation - use base gain only
        tokens_adapt = self._match_mod_dtype(tokens, self.cond_adapter).clone()
        tokens_adapt[:, 0, :] = tokens_adapt[:, 0, :] * 1.5
        base_gain = self._adapter_gain_scale()
        cond_patch = self.cond_adapter(tokens_adapt, T_out=x_noisy.shape[-1], scale=base_gain)
        cond_patch = cond_patch.to(device=x_noisy.device, dtype=x_noisy.dtype)

        # Apply pitch-height masking for tighter pitch lock (same as training)
        B, C, H, T_lat = x0.shape
        pr = batch["conds"]["piano_roll"].to(device=x0.device, dtype=x0.dtype)
        if pr.shape[-1] != T_lat:
            pr = F.interpolate(pr, size=T_lat, mode="nearest")
        ...
        # Time-weighted recon term (same as training but no outlier filtering)
        x0_hat = x_noisy - sigma * v_pred
        pr_tgt = batch["conds"]["piano_roll"].to(x0_hat.device, dtype=x0_hat.dtype)
        if pr_tgt.shape[-1] != x0_hat.shape[-1]:
            pr_tgt = F.interpolate(pr_tgt, size=x0_hat.shape[-1], mode="nearest")
        
        err_bt = (x0_hat - x0).pow(2).mean(dim=(1,2))  # [B,T]
        time_w = 1.0 + 0.5 * (pr_tgt.amax(dim=1) > 0).to(x0_hat.dtype)
        recon_v = (err_bt * time_w).mean()             # scalar
        loss = v_loss + 0.25 * recon_v + aux_w * aux_loss

        # Log validation metrics
        self.log("val/loss", loss, on_step=True)
        self.log("val/v_loss", v_loss, on_step=True)
