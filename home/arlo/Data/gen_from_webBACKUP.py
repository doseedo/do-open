...
- Simple CFG option (unconditional pass disables residuals per step)

Usage (example):
  CUDA_VISIBLE_DEVICES=0 \
  PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  python web_ui_performer.py \
...
def generate(
    ...
    # Temporarily modify the conditioning encoder's instrument strength
    original_inst_strength = getattr(model.ctrl_enc, 'inst_strength', 3.0)
    if instrument_strength != 1.0:
        # Scale the original strength by our multiplier
        model.ctrl_enc.inst_strength = original_inst_strength * float(instrument_strength)
        print(f"🎛️ Modified ctrl_enc.inst_strength: {original_inst_strength} -> {model.ctrl_enc.inst_strength}")
    
    tokens, _ = model.ctrl_enc(**conds)
    
    # Restore original strength after getting tokens
    if instrument_strength != 1.0:
        model.ctrl_enc.inst_strength = original_inst_strength
    ...
    for i in range(steps, 0, -1):
        ...
        # Compute adapter patch each step (like in training)
        tokens_adapt = tokens.to(dtype=next(model.cond_adapter.parameters()).dtype).clone()
        tokens_adapt[:, 0, :] = tokens_adapt[:, 0, :] * 1.5  # Token enhancement
        ...
        cond_patch = cond_patch * float(adapter_scale)
        
        # Apply pitch→height mask each step (like in training)
        B, C, H, T_lat = x.shape
        ...
        # Toggle ControlBranch residuals depending on CFG branch
        if cfg_weight <= 1.0:
            # single conditional pass with residuals (if any)
            if ctrl_res_list is not None:
                model._ctrl_residuals = ctrl_res_list
            v_pred = model._call_transformer_no_xattn(latents=x + cond_patch, t=t_idx)
        else:
            # unconditional (no residuals)
            model._ctrl_residuals = None
            v_uncond = model._call_transformer_no_xattn(latents=x, t=t_idx)

            # conditional (with residuals)
            if ctrl_res_list is not None:
                model._ctrl_residuals = ctrl_res_list
            v_cond   = model._call_transformer_no_xattn(latents=x + cond_patch, t=t_idx)

            v_pred = v_uncond + cfg_weight * (v_cond - v_uncond)
...
def run_generation(
    ...
    instrument_strength, piano_roll_gain, amp_gain, rframe_gain, rbend_gain, encodec_gain, use_overlap_decoder, progress=gr.Progress(track_tqdm=True)
):
    ...
    out = generate(
        ...
        sr_out=32000, instrument_strength=float(instrument_strength), piano_roll_gain=float(piano_roll_gain), amp_gain=float(amp_gain), 
...
def create_ui():
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as iface:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    ...
                    cfg_slider     = gr.Slider(1.0, 15.0, value=1.0, step=0.1, label="CFG (Guidance Strength)")
                ...
                with gr.Row():
                    piano_roll_gain = gr.Slider(0.0, 2.0, value=1.0, step=0.1, label="Piano Roll Gain")
                ...
                gr.Markdown("### 5. Audio Decoder Options")
        ...
        all_inputs = [audio_input, group_dd, subgroup_dd, seed_slider, steps_slider, adapter_slider, cfg_slider, t0_slider, 
                     instrument_strength, piano_roll_gain, amp_gain, rframe_gain, rbend_gain, encodec_gain, use_overlap_decoder]
