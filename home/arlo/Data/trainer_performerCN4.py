...
from dataloader_no_encodec import PerformerAIDatasetNoEncodec, collate_latent_cond_no_encodec
from conditioning_encoder_no_encodec import PerformanceConditionEncoderNoEncodec
import torchaudio
...
#TRAIN FROM SCRATCH
        ...
        # ---- ctrl_enc now gets the correct sizes (NO ENCODEC VERSION) ----
        self.ctrl_enc = PerformanceConditionEncoderNoEncodec(
            d_text=d_text,
    ...
    def on_fit_start(self):
        ...
        ds_prev = PerformerAIDatasetNoEncodec(
        ...
        item = ds_prev[idx]
        self._preview_batch = collate_latent_cond_no_encodec([item])  # stays on CPU; preview fns .to(self.device)
        ...
        try:
            ...
            ds_check = PerformerAIDatasetNoEncodec(
                json_path=self.manifest_json,
...
#TEST DATALOADER:
        ...
        ds = PerformerAIDatasetNoEncodec(
            json_path=self.manifest_json,
            conditioning_dropout={"piano_roll":0.15, "amp":0.10, "rbend":0.10, "rframe":0.05},
            use_trim=True,
            pre_roll_seconds=1.0,
        ...
        try:
            ...
            # Calculate subgroup frequencies for light weighting
            from collections import Counter
            subgroup_counts = Counter()
            for it in data:
                sub_name = (it.get("sub_group") or it.get("subgroup") or "unknown").lower()
                subgroup_counts[sub_name] += 1

            # Compute combined group + subgroup weights
            alpha = 0.3  # Light subgroup weighting (0=no weight, 1=full inverse frequency)
            w = []
            for i, it in enumerate(data):
                ...
                # Group weight (full strength)
                group_weight = 1.0 / hours.get(gid_name, 1.0)

                # Subgroup weight (light strength via alpha)
                sub_count = subgroup_counts.get(sub_name, 1)
                sub_weight = (1.0 / sub_count) ** alpha

                # Combine: multiply group and subgroup weights
                w.append(group_weight * sub_weight)
            ...
            print(f"[balance] Using weighted sampling with light subgroup boost (alpha={alpha})")
            print(f"  Hours per group: {dict(hours)}")
            for g, h in sorted(hours.items()):
                weight_boost = (1.0 / h) / (sum(1.0/v for v in hours.values()) / len(hours))
                print(f"    {g}: {h:.1f}h -> {weight_boost:.2f}x group sampling boost")
            print(f"  Top 10 rarest subgroups (sample counts):")
            for sub_name, count in subgroup_counts.most_common()[-10:]:
                boost = (1.0 / count) ** alpha
                print(f"    {sub_name}: {count} samples -> {boost:.2f}x subgroup boost")
        ...
        return DataLoader(
            ...
            collate_fn=collate_latent_cond_no_encodec,
...
# OG ACE STEP DIFFUSION
    ...
    def training_step(self, batch, batch_idx):
        ...
        # controls (NO ENCODEC VERSION - removed encodec swapping and dropout)
