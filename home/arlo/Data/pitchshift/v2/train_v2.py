"""
Register-Aware Pitch Shift Training V2
...
from dataset_v2 import RegisterTransferDatasetV2
from models_v2 import RegisterTranslator, RegisterTranslatorDirect


class TrainerV2:
    """
    Trainer for V2 register timbre conversion using DISTRIBUTION MATCHING.
    ...
    def __init__(
        ...
        self.dataset = RegisterTransferDatasetV2(
        ...
        config = {
            ...
            'version': 'v2-distribution-matching',
        }
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch in pbar:
            ...
            # Combined loss (like mute_translator)
            # For RESIDUAL models: NO content loss - residual connection preserves it
            # For DIRECT models: ADD content loss (no passthrough)
            if self.model_type == 'direct':
                total = self.dist_weight * dist_loss + 0.5 * content_loss + 1.0 * silence_loss
            else:
                # Residual model: distribution + strong silence only
                total = self.dist_weight * dist_loss + 1.0 * silence_loss
    ...
    def train(self):
        ...
        print("\n" + "=" * 60)
        print("REGISTER TIMBRE V2 - DISTRIBUTION MATCHING")
        print("=" * 60)
        ...
        print("Like mute_translator - distribution matching!")
        print("=" * 60 + "\n")
...
def main():
    parser = argparse.ArgumentParser(description="Train Register Timbre V2")
    ...
    parser.add_argument('--dist_weight', type=float, default=0.5,
                        help='Weight for distribution loss (lower = more content preservation)')
