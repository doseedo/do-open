...
class C3FocusedDistributionLoss(nn.Module):
    """
    Distribution matching with heavy C3 weighting.

    Based on probe finding: C3 encodes most register information.
    DSP-shifted C3 is -4.03 from natural target, while natural register
    diff is +2.52. C3 is THE lever for register correction.

    Other channels get 1x weight, C3 gets c3_weight (default 10x).
    """

    def __init__(self, c3_weight: float = 10.0):
        super().__init__()
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> dict:
        ...
        # === C3 FOCUSED LOSSES (the main signal) ===
        pred_c3 = pred[:, 3:4, :, :]  # [B, 1, H, T]
        target_c3 = target[:, 3:4, :, :]

        # C3 energy matching (this is the big one)
        pred_c3_energy = (pred_c3 ** 2).mean()
        target_c3_energy = (target_c3 ** 2).mean()
        c3_energy_loss = F.mse_loss(pred_c3_energy, target_c3_energy)

        # C3 mean matching
        pred_c3_mean = pred_c3.mean()
        target_c3_mean = target_c3.mean()
        c3_mean_loss = F.mse_loss(pred_c3_mean, target_c3_mean)

        # C3 std matching
        pred_c3_std = pred_c3.std()
        target_c3_std = target_c3.std()
        c3_std_loss = F.mse_loss(pred_c3_std, target_c3_std)

        # C3 per-H profile (does C3 have the right shape along H?)
        pred_c3_h_profile = pred_c3.mean(dim=(0, 1, 3))  # [H]
        target_c3_h_profile = target_c3.mean(dim=(0, 1, 3))
        c3_profile_loss = F.mse_loss(pred_c3_h_profile, target_c3_h_profile)

        c3_total = c3_energy_loss + c3_mean_loss + c3_std_loss + 0.5 * c3_profile_loss

        # === OTHER CHANNELS (lighter weight) ===
        other_indices = [0, 1, 2, 4, 5, 6, 7]
        pred_other = pred[:, other_indices, :, :]
        target_other = target[:, other_indices, :, :]

        # Per-channel stats for other channels
        pred_other_mean = pred_other.mean(dim=(2, 3))  # [B, 7]
        target_other_mean = target_other.mean(dim=(2, 3))
        other_mean_loss = F.mse_loss(pred_other_mean, target_other_mean)

        pred_other_std = pred_other.std(dim=(2, 3))
        target_other_std = target_other.std(dim=(2, 3))
        other_std_loss = F.mse_loss(pred_other_std, target_other_std)

        other_total = other_mean_loss + other_std_loss

        # === COMBINED ===
        total = self.c3_weight * c3_total + other_total

        return {
            'total': total,
            'c3_energy': c3_energy_loss,
            'c3_mean': c3_mean_loss,
            'c3_std': c3_std_loss,
            'c3_profile': c3_profile_loss,
            'c3_total': c3_total,
...
class LoudnessLoss(nn.Module):
    """
    V5: Hard loudness lock between pred and SOURCE.

    Kills the "just boost everything" shortcut.
    Uses RMS matching (global and per-frame).
    """
    ...
    def forward(self, pred: torch.Tensor, source: torch.Tensor) -> torch.Tensor:
        # Global RMS match
        pred_rms = pred.pow(2).mean().sqrt()
        source_rms = source.pow(2).mean().sqrt()
        global_loss = F.mse_loss(pred_rms, source_rms)

        # Per-frame RMS match (preserves envelope)
        pred_frame_rms = pred.pow(2).mean(dim=(1, 2)).sqrt()  # [B, T]
        source_frame_rms = source.pow(2).mean(dim=(1, 2)).sqrt()
        frame_loss = F.mse_loss(pred_frame_rms, source_frame_rms)

        return global_loss + frame_loss


class EnvelopeLoss(nn.Module):
    """
    V5: Spectral envelope loss.

    Smooths along H dimension to capture formant/envelope shape,
    ignoring fine harmonic structure.
    Compares smoothed log-magnitude profiles.
    """

    def __init__(self, smooth_kernel: int = 3):
        super().__init__()
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Log magnitude (with floor for stability)
        pred_log = torch.log(pred.abs() + 1e-6)
        target_log = torch.log(target.abs() + 1e-6)

        # Smooth along H dimension (spectral envelope)
        # Use avg pool to smooth
        pred_smooth = F.avg_pool2d(
            pred_log,
            kernel_size=(self.smooth_kernel, 1),
            stride=1,
            padding=(self.smooth_kernel // 2, 0)
        )
        target_smooth = F.avg_pool2d(
            target_log,
            kernel_size=(self.smooth_kernel, 1),
            stride=1,
            padding=(self.smooth_kernel // 2, 0)
        ...
        # Normalize to compare shape (not absolute level)
        pred_norm = pred_smooth - pred_smooth.mean(dim=2, keepdim=True)
        target_norm = target_smooth - target_smooth.mean(dim=2, keepdim=True)
