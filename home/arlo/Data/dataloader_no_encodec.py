# dataloader_no_encodec.py
# Version that doesn't load encodec tokens - for conditioning without timbre
import json, random
...
def _pad_last(x: torch.Tensor, target_len: int) -> torch.Tensor:
    cur = x.shape[-1]
    if cur == target_len: return x
    if cur < target_len:  return F.pad(x, (0, target_len - cur))
    return x[..., :target_len]
...
class PerformerAIDatasetNoEncodec(Dataset):
    """Dataset that doesn't load encodec tokens - for training without timbre conditioning"""
    def __init__(self,
                 json_path: str,
        ...
        if self.require_all_core:
            ...
            for it in self.manifest:
                ...
                # REMOVED: encodec check - we don't need it
...
def collate_latent_cond_no_encodec(batch: List[Dict[str,Any]]) -> Dict[str,Any]:
    """Collate function without encodec tokens"""
