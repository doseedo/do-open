...
class MusicDCAE(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def decode_overlap(self, latents, audio_lengths=None, sr=None):
        ...
        dcae_win_len_latent = 1024  # Increased from 512 for faster processing (fewer windows)
        ...
        dcae_anchor_hop = dcae_win_len_latent * 3 // 4  # 75% hop instead of 50% (less overlap, faster)
        ...
        vocoder_win_len_audio = 512 * 1024  # Doubled from 512*512 for fewer vocoder passes
        # vocoder_overlap_len_audio: Audio samples for overlap between vocoder windows
        vocoder_overlap_len_audio = 512  # Reduced from 1024 for less overlap  
