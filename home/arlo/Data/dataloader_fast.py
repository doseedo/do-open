# dataloader_fast.py  — derived from dataloader.py
# Accepts unified_manifest.json (dict with "entries") or old list-of-dicts format.
# Derives conditioning/piano_roll paths from audio_path when not explicitly provided.
import json, random, os, orjson
from pathlib import Path
from typing import Dict, Any, List, Optional

GCS_BUCKET = "/home/arlo/gcs-bucket"
CONDITIONING_ROOT = os.path.join(GCS_BUCKET, "Conditioning")

def _derive_cond_stem(audio_path: str) -> str:
    """audio_path -> relative stem under gcs-bucket for conditioning lookups."""
    rel = audio_path.replace(GCS_BUCKET + "/", "")
    return os.path.splitext(rel)[0]

def _enrich_entry(e: Dict[str, Any]) -> Dict[str, Any]:
    """Fill in conditioning_paths / piano_roll_path from audio_path if missing."""
    # normalise subgroup field name
    if "sub_group" not in e and "subgroup" in e:
        e["sub_group"] = e["subgroup"]
    ap = e.get("audio_path", "")
    if not ap:
        return e
    stem = _derive_cond_stem(ap)
    cond_base = os.path.join(CONDITIONING_ROOT, stem)
    # fill conditioning_paths if absent
    if not e.get("conditioning_paths"):
        e["conditioning_paths"] = {
            "amp":    cond_base + ".amp.npy",
            "rframe": cond_base + ".rframe.npy",
            "rbend":  cond_base + ".rbend.npy",
            "onsets": cond_base + ".onsets.npy",
            "f0":     cond_base + ".f0.npy",
        }
    # fill piano_roll_path if absent
    if not e.get("piano_roll_path"):
        e["piano_roll_path"] = cond_base + ".pianoroll.npy"
    return e
...
class PerformerAIDataset(Dataset):
    def __init__(self,
        ...
        super().__init__()
        raw = orjson.loads(Path(json_path).read_bytes())
        # unified manifest is a dict with "entries"; old format is a plain list
        if isinstance(raw, dict):
            entries = raw.get("entries", [])
        else:
            entries = raw
        # enrich each entry with derived paths
        self.manifest: List[Dict[str, Any]] = [_enrich_entry(e) for e in entries]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        subgroup = (meta.get("sub_group") or meta.get("subgroup") or "undefined").lower()
