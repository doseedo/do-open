...
def detect_key_from_midi(midi_path: str) -> dict:
    """
    Detect which pitch classes (notes) are used in a MIDI file.
    ...
    Returns:
        dict: {
            'pitch_classes': set of used pitch classes (0-11),
            'pitches': set of all MIDI pitches used (0-127),
            'note_counts': dict mapping pitch to count,
            'dominant_octaves': list of most used octave ranges
        }
    """
    midi_data = pretty_midi.PrettyMIDI(midi_path)
    ...
    # Find dominant octaves (octaves with most notes)
    octave_counts = {}
    for pitch in pitches:
        octave = pitch // 12
        octave_counts[octave] = octave_counts.get(octave, 0) + note_counts.get(pitch, 0)

    dominant_octaves = sorted(octave_counts.keys(), key=lambda o: octave_counts[o], reverse=True)[:3]

    # Create pitch class names for debugging
    pitch_class_names = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
    used_notes = [pitch_class_names[pc] for pc in sorted(pitch_classes)]

    print(f"🎵 Key Detection: Found {len(pitch_classes)} pitch classes: {', '.join(used_notes)}")
    print(f"   Pitch range: {min(pitches)}-{max(pitches)}, Dominant octaves: {dominant_octaves}")

    return {
        'pitch_classes': pitch_classes,
        'pitches': pitches,
        'note_counts': note_counts,
        'dominant_octaves': dominant_octaves,
        'used_notes': used_notes
    }

def create_key_reference_piano_roll(midi_path: str, window_slow: int = 1024,
                                     key_pattern_type: str = "constant",
                                     reference_intensity: float = 0.3) -> tuple:
    """
    Create a key reference piano roll that shows which notes are in the key,
    instead of the actual melody timing.

    Args:
        midi_path: Path to MIDI file
        window_slow: Target length for conditioning
        key_pattern_type: Type of reference pattern:
            - "constant": Constant activation for all notes in key
            - "pulse": Pulsing pattern for notes in key
            - "sparse": Very sparse occasional hints
        reference_intensity: Intensity of the reference (0-1)

    Returns:
        tuple: (key_reference_piano_roll, empty_amp, empty_rframe, empty_rbend, empty_encodec)
    """
    print(f"🎼 Creating key reference piano roll (mode: {key_pattern_type}): {Path(midi_path).name}")

    # Detect which notes are in the key
    key_info = detect_key_from_midi(midi_path)
    pitches = key_info['pitches']
    pitch_classes = key_info['pitch_classes']

    # Create piano roll with target length
    piano_roll = np.zeros((128, window_slow))

    if key_pattern_type == "constant":
        # Constant low-intensity activation for all pitches in the key
        for pitch in pitches:
            piano_roll[pitch, :] = reference_intensity
        print(f"   Created constant reference for {len(pitches)} pitches")

    elif key_pattern_type == "pulse":
        # Pulsing pattern - activate every N frames
        pulse_interval = 32  # Pulse every 32 frames (~0.74s at 43fps)
        for pitch in pitches:
            for i in range(0, window_slow, pulse_interval):
                pulse_width = min(4, window_slow - i)  # 4-frame pulse
                piano_roll[pitch, i:i+pulse_width] = reference_intensity
        print(f"   Created pulsing reference for {len(pitches)} pitches (interval: {pulse_interval})")

    elif key_pattern_type == "sparse":
        # Very sparse hints - just a few activations throughout
        num_hints = 5  # Number of hint points
        for pitch in pitches:
            for i in range(num_hints):
                hint_pos = (i * window_slow) // num_hints
                hint_width = min(2, window_slow - hint_pos)
                piano_roll[pitch, hint_pos:hint_pos+hint_width] = reference_intensity * 0.5
        print(f"   Created sparse reference for {len(pitches)} pitches ({num_hints} hints)")

    elif key_pattern_type == "pitch_class":
        # Activate all instances of the pitch classes across all octaves
        for pc in pitch_classes:
            for octave in range(11):  # MIDI has 11 octaves (0-10)
                pitch = pc + (octave * 12)
                if pitch < 128:
                    piano_roll[pitch, :] = reference_intensity * 0.7
        print(f"   Created pitch class reference for {len(pitch_classes)} pitch classes across all octaves")

    else:
        raise ValueError(f"Unknown key_pattern_type: {key_pattern_type}")

    # Create empty conditioning for other modalities
    empty_amp = np.zeros(window_slow)
    empty_rframe = np.zeros(window_slow)
    empty_rbend = np.zeros(window_slow)

    # Create minimal encodec tokens
    encodec_length = window_slow // 4
    empty_encodec = torch.zeros((1, 8, encodec_length), dtype=torch.long)

    print(f"✅ Key reference ready: PR={piano_roll.shape}, intensity={reference_intensity}")
...
def generate(
    ...
    x_pre_decode = x.clone().cpu() if return_latents else None
    ...
    if return_latents:
        return str(out_path), x_pre_decode
    return str(out_path)
...
def run_generation(
    ...
    key_conditioning_mode, key_pattern_type, key_reference_intensity,
    ...
    if is_midi and midi_mode:
        ...
        # Key Conditioning Mode: Use only key reference, no MIDI melody
        if key_conditioning_mode:
            progress(0.1, desc="Creating key reference (no MIDI melody)…")
            pr, amp, rfr, rbd, enc = create_key_reference_piano_roll(
                audio_file, win_slow,
                key_pattern_type=key_pattern_type,
                reference_intensity=key_reference_intensity
            )
            orig_len = None
            print(f"🎹 Mode: KEY CONDITIONING - Model will freely generate melodies using key reference only")
            print(f"   Pattern: {key_pattern_type}, Intensity: {key_reference_intensity}")

        elif render_and_extract:
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as iface:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                # Key Conditioning Mode (NEW!)
                gr.Markdown("#### 🎹 Key Conditioning Mode")
                key_conditioning_mode = gr.Checkbox(
                    label="Enable Key Conditioning Mode",
                    value=False,
                    info="Use MIDI only as key reference - model freely generates melodies in that key"
                )
                with gr.Row():
                    key_pattern_type = gr.Dropdown(
                        choices=["constant", "pulse", "sparse", "pitch_class"],
                        label="Key Pattern Type",
                        value="constant",
                        info="How key reference is presented to the model"
                    )
                    key_reference_intensity = gr.Slider(
                        0.0, 1.0,
                        value=0.3,
                        step=0.05,
                        label="Key Reference Intensity",
                        info="Strength of key conditioning hint"
                    )
