...
from dataloader_fast import APPROVED_GROUPS, APPROVED_SUBGROUPS
...
from dataloader_fast import PerformerAIDataset, collate_latent_cond
...
class TokenSummarizer(nn.Module):
    ...
    Output: summary [B, D]
    """
    def __init__(self, d_text: int, kernel_size: int = 3):
        super().__init__()
        self.ln = nn.LayerNorm(d_text)
        # depthwise temporal conv (groups=D) keeps channels independent
        # kernel_size=3 (~280ms at SLOW_HZ) to preserve fast articulation detail
...
class TemporalCondAdapter(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, T_out: int, scale: float = 1.0) -> torch.Tensor:
        ...
        f = F.interpolate(f, size=T_out, mode="nearest")  # [B, D, T_out]  -- nearest preserves sharp onsets
...
class ControlBranch1D(nn.Module):
    ...
    def forward(self, pr_128t: torch.Tensor, T_out_list: list[int], instrument_token=None):
        ...
        for proj, T_i in zip(self.to_blocks, T_out_list):
            h_i = F.interpolate(h, size=T_i, mode="nearest")
            feat = proj(h_i)  # [B, C_i, T_i]
            if self.with_importance:
                m = torch.sigmoid(F.interpolate(self.imp(h), size=T_i, mode="nearest"))  # [B,1,T_i]
                feat = feat * m
            outs.append(feat)
        return outs
...
def main(args):
    ...
    if args.ckpt_path and args.weights_only:
        ...
        # Kernel surgery: slice old kernel_size=9 dw_conv weight → new kernel_size=3
        dw_key = "token_summary.dw_conv.weight"
        if dw_key in sd:
            old_w = sd[dw_key]  # [D, 1, 9]
            new_ks = model.token_summary.dw_conv.kernel_size[0]
            old_ks = old_w.shape[-1]
            if old_ks != new_ks:
                center = old_ks // 2
                half = new_ks // 2
                sd[dw_key] = old_w[:, :, center - half : center + half + 1]
                print(f"[kernel-surgery] sliced {dw_key}: kernel {old_ks} -> {new_ks} (center crop)")
