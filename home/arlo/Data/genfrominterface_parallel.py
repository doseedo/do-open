#!/usr/bin/env python3
"""
Gradio Web UI for ACE-Step generation (ControlBranch-ready) - PARALLEL VERSION
...
- PARALLEL: Multiprocessing for FluidSynth rendering and CPU-bound tasks
"""
...
# Parallel processing pool for CPU-bound tasks
CPU_WORKER_POOL: Union[ProcessPoolExecutor, None] = None
MAX_CPU_WORKERS: int = mp.cpu_count() - 1  # Leave one core free for main process

# GPU batching configuration
GPU_BATCH_SIZE: int = 4  # Default batch size for GPU generation
AUTO_BATCH_SIZE: bool = True  # Automatically determine optimal batch size

# ------------------------------------------------------------------------------
# GPU Memory Management
# ------------------------------------------------------------------------------
def get_gpu_memory_info(device: Union[str, torch.device] = "cuda:0") -> dict:
    """
    ...
    Returns:
        dict: {
            ...
            'allocated': Currently allocated in GB,
            'reserved': Reserved by PyTorch in GB,
            'free': Approximate free memory in GB
        }
    """
...
def estimate_batch_size(model: Pipeline, sample_shape: tuple, safety_factor: float = 0.7) -> int:
    """
    Estimate optimal batch size based on available GPU memory.

    Args:
        model: The Pipeline model
        sample_shape: Shape of a single sample (C, H, T)
        safety_factor: Use this fraction of free memory (default 0.7 for safety)

    Returns:
        int: Recommended batch size (minimum 1, maximum 8)
    """
    ...
    free_gb = mem_info['free'] * safety_factor

    # Rough estimate: each sample uses ~4GB during generation
    # This includes forward pass, CFG (2x), gradients, etc.
    estimated_gb_per_sample = 4.0

    # Calculate how many samples can fit
    batch_size = max(1, int(free_gb / estimated_gb_per_sample))

    # Cap at reasonable maximum to avoid OOM
    batch_size = min(batch_size, 8)

    print(f"📊 GPU Memory: {mem_info['free']:.1f}GB free / {mem_info['total']:.1f}GB total")
    print(f"🎯 Estimated batch size: {batch_size} (using {safety_factor*100:.0f}% of free memory)")
...
# ------------------------------------------------------------------------------
def init_cpu_worker_pool(max_workers: int = None):
    """Initialize the CPU worker pool for parallel processing."""
    global CPU_WORKER_POOL, MAX_CPU_WORKERS
    if CPU_WORKER_POOL is not None:
        print("⚠️ CPU worker pool already initialized")
        return
    ...
    CPU_WORKER_POOL = ProcessPoolExecutor(max_workers=max_workers)
    print(f"✅ Initialized CPU worker pool with {max_workers} workers")

def shutdown_cpu_worker_pool():
    """Shutdown the CPU worker pool."""
    global CPU_WORKER_POOL
    if CPU_WORKER_POOL is not None:
        CPU_WORKER_POOL.shutdown(wait=True)
        CPU_WORKER_POOL = None
        print("🛑 CPU worker pool shut down")

def render_midi_worker(midi_path: str, output_dir: str, instrument_group: str) -> str:
    """
    Worker function for parallel MIDI rendering. This runs in a separate process.
    ...
    try:
        ...
        soundfont_path = INSTRUMENT_SOUNDFONTS.get("default")
        ...
        print(f"🎵 [Worker] Rendering MIDI: {midi_path.name} -> {audio_filename}")
        ...
        try:
            ...
            if result.returncode == 0 and audio_path.exists():
                print(f"✅ [Worker] FluidSynth rendering successful: {audio_path}")
                return str(audio_path)
            else:
                print(f"⚠️ [Worker] FluidSynth failed, trying fallback...")
                if audio_path.exists():
                    audio_path.unlink()

        except (subprocess.TimeoutExpired, FileNotFoundError) as e:
            print(f"⚠️ [Worker] FluidSynth error: {e}, trying fallback...")

        # Fallback: Create sine wave audio
        try:
            print("🔄 [Worker] Fallback: Creating sine wave audio...")
            ...
            print(f"✅ [Worker] Sine wave synthesis successful: {audio_path}")
            return str(audio_path)

        except Exception as e:
            print(f"❌ [Worker] All rendering methods failed: {e}")
            raise RuntimeError(f"Could not render MIDI to audio: {e}")

    except Exception as e:
        print(f"❌ [Worker] Fatal error in render_midi_worker: {e}")
        raise
...
def render_midi_batch_parallel(midi_paths: list, output_dir: str = "./temp_audio",
                                instrument_groups: list = None, use_parallel: bool = True) -> list:
    """
    Render multiple MIDI files to audio in parallel using the CPU worker pool.

    Args:
        midi_paths: List of MIDI file paths to render
        output_dir: Directory to save rendered audio files
        instrument_groups: List of instrument groups (one per MIDI file), or None
        use_parallel: If True, use parallel processing. If False, render sequentially.

    Returns:
        List of rendered audio file paths (in same order as input)
    """
    global CPU_WORKER_POOL
    ...
    # Ensure instrument_groups is a list with same length as midi_paths
    if instrument_groups is None:
        instrument_groups = [None] * len(midi_paths)
    elif len(instrument_groups) < len(midi_paths):
        instrument_groups.extend([None] * (len(midi_paths) - len(instrument_groups)))

    # If parallel processing is disabled or pool is not initialized, render sequentially
    if not use_parallel or CPU_WORKER_POOL is None:
        print(f"🎵 Rendering {len(midi_paths)} MIDI files sequentially...")
        results = []
        for midi_path, inst_group in zip(midi_paths, instrument_groups):
            try:
                audio_path = render_midi_to_audio(midi_path, output_dir, inst_group)
                results.append(audio_path)
            except Exception as e:
                print(f"❌ Failed to render {midi_path}: {e}")
                results.append(None)
        return results

    # Parallel processing using worker pool
    print(f"🚀 Rendering {len(midi_paths)} MIDI files in parallel (up to {MAX_CPU_WORKERS} workers)...")
    start_time = time.time()

    # Submit all rendering tasks to the worker pool
    futures = []
    for midi_path, inst_group in zip(midi_paths, instrument_groups):
        future = CPU_WORKER_POOL.submit(render_midi_worker, midi_path, output_dir, inst_group)
        futures.append((midi_path, future))

    # Collect results as they complete
    results = [None] * len(midi_paths)
    completed = 0

    for idx, (midi_path, future) in enumerate(futures):
        try:
            audio_path = future.result(timeout=120)  # 2 minute timeout per file
            results[idx] = audio_path
            completed += 1
            print(f"✅ [{completed}/{len(midi_paths)}] Completed: {Path(midi_path).name}")
        except Exception as e:
            print(f"❌ [{completed+1}/{len(midi_paths)}] Failed: {Path(midi_path).name} - {e}")
            results[idx] = None
            completed += 1

    elapsed = time.time() - start_time
    print(f"🎉 Parallel rendering complete: {completed}/{len(midi_paths)} files in {elapsed:.2f}s")
    print(f"   Average: {elapsed/len(midi_paths):.2f}s per file")
...
def process_multitrack_midi_simple(midi_path: str, progress=None, **generation_args) -> tuple:
    """
    Process multitrack MIDI using the simple approach with PARALLEL PROCESSING:
    1. Split MIDI into individual track files
    2. Render each track to FluidSynth audio (PARALLEL)
    3. Extract conditioning from audio files (PARALLEL)
    4. Generate audio from conditioned tracks (Sequential - GPU bound)
    5. Sum the results
    ...
    print(f"🎼 Processing multitrack MIDI with PARALLEL processing: {Path(midi_path).name}")
    if CPU_WORKER_POOL is not None:
        print(f"   ✅ Worker pool active with {MAX_CPU_WORKERS} workers")
    else:
        print(f"   ⚠️ WARNING: Worker pool not initialized - falling back to sequential processing")
    ...
    if progress:
        progress(0.2, desc="Rendering tracks to audio in parallel...")

    # Step 2: Render each track to FluidSynth audio - USING PARALLEL PROCESSING
    print(f"🚀 Rendering {track_count} tracks in parallel...")
    instrument_subgroup = generation_args.get('subgroup', None)
    instrument_groups = [instrument_subgroup] * track_count

    # Use parallel batch rendering
    individual_audio_files = render_midi_batch_parallel(
        track_midi_files,
    ...
    individual_audio_files = [f for f in individual_audio_files if f is not None]

    if len(individual_audio_files) != track_count:
        print(f"⚠️ Warning: {track_count - len(individual_audio_files)} tracks failed to render")

    print(f"✅ Parallel rendering complete: {len(individual_audio_files)} tracks rendered")

    if progress:
        progress(0.4, desc="Extracting conditioning in parallel...")

    # Step 3a: Extract conditioning for all tracks in parallel (CPU-bound)
    print(f"🚀 Extracting conditioning for {len(individual_audio_files)} tracks in parallel...")
    instrument_subgroup = generation_args.get('subgroup', None)
    instrument_groups = [instrument_subgroup] * len(individual_audio_files)

    # Use parallel batch conditioning extraction
    conditioning_results = extract_conditioning_batch_parallel(
        individual_audio_files,
        output_dir="./extracted_conditioning",
        instrument_groups=instrument_groups,
        use_parallel=True
    )

    print(f"✅ Parallel conditioning extraction complete: {len(conditioning_results)} tracks processed")

    if progress:
        progress(0.5, desc="Generating audio from tracks...")

    # Step 3b: Generate audio from each conditioned track (GPU-bound, must be sequential)
    print(f"🎼 Generating audio from {len(conditioning_results)} tracks (sequential, GPU-bound)...")
    individual_outputs = []
    for i, (audio_file, extraction) in enumerate(zip(individual_audio_files, conditioning_results)):
        if progress:
            progress(0.5 + (i / len(conditioning_results)) * 0.4, desc=f"Generating track {i+1}/{len(conditioning_results)}...")

        print(f"   Generating track {i+1}: {Path(audio_file).name}")

        # Load conditioning that was already extracted
...
def render_multitrack_debug_audio(track_midi_paths: list, debug_dir: str, audio_stem: str, instrument_group: str = None) -> list:
    ...
    # Filter out invalid MIDI paths
    valid_midi_paths = [p for p in track_midi_paths if p and Path(p).exists()]

    if not valid_midi_paths:
        print("⚠️ No valid MIDI files to render")
        return []

    # Use parallel batch rendering for all tracks
    instrument_groups = [instrument_group] * len(valid_midi_paths)
    rendered_audio_paths = render_midi_batch_parallel(
        valid_midi_paths,
    ...
    # Filter out None results
    rendered_audio_paths = [p for p in rendered_audio_paths if p is not None]
...
def extract_conditioning_worker(audio_path: str, output_dir: str, instrument_group: str = None) -> dict:
    """
    Worker function for parallel conditioning extraction.
    ...
    try:
        ...
        req = [out_dir/f"{base_stem}.pianoroll.npy", out_dir/f"{base_stem}.amp.npy",
               out_dir/f"{base_stem}.rframe.npy", out_dir/f"{base_stem}.rbend.npy",
               out_dir/f"{base_stem}.encodec.pt"]

        if all(x.exists() for x in req):
            print(f"✅ [Worker] Conditioning already exists: {Path(audio_path).name}")
            return {"dir": str(out_dir), "stem": base_stem, "audio_path": audio_path}

        # Extract new conditioning
        print(f"🔄 [Worker] Extracting conditioning for: {Path(audio_path).name}")
        cmd = ["python", "test_extract_local.py", "--input", str(audio_path), "--output", str(out_dir)]
        res = subprocess.run(cmd, capture_output=True, text=True, timeout=300)

        if res.returncode != 0:
            print(f"❌ [Worker] Extraction failed for {Path(audio_path).name}")
            print(res.stdout)
            print(res.stderr)
            raise RuntimeError(f"Extraction failed for {audio_path}")

        print(f"✅ [Worker] Conditioning extracted: {Path(audio_path).name}")
        return {"dir": str(out_dir), "stem": base_stem, "audio_path": audio_path}

    except Exception as e:
        print(f"❌ [Worker] Error extracting {audio_path}: {e}")
        raise

def extract_conditioning_batch_parallel(audio_paths: list, output_dir: str = "./extracted_conditioning",
                                         instrument_groups: list = None, use_parallel: bool = True) -> list:
    """
    Extract conditioning from multiple audio files in parallel using CPU worker pool.

    Args:
        audio_paths: List of audio file paths to extract conditioning from
        output_dir: Base directory for extracted conditioning
        instrument_groups: List of instrument groups (one per audio file), or None
        use_parallel: If True, use parallel processing. If False, extract sequentially.

    Returns:
        List of extraction result dicts (in same order as input)
    """
    global CPU_WORKER_POOL
    ...
    # Ensure instrument_groups is a list with same length as audio_paths
    if instrument_groups is None:
        instrument_groups = [None] * len(audio_paths)
    elif len(instrument_groups) < len(audio_paths):
        instrument_groups.extend([None] * (len(audio_paths) - len(instrument_groups)))

    # If parallel processing is disabled or pool is not initialized, extract sequentially
    if not use_parallel or CPU_WORKER_POOL is None:
        print(f"🔄 Extracting conditioning for {len(audio_paths)} files sequentially...")
        results = []
        for audio_path, inst_group in zip(audio_paths, instrument_groups):
            try:
                result = extract_conditioning_from_audio(audio_path, output_dir, inst_group)
    ...
    # Parallel processing using worker pool
    print(f"🚀 Extracting conditioning for {len(audio_paths)} files in parallel (up to {MAX_CPU_WORKERS} workers)...")
    start_time = time.time()

    # Submit all extraction tasks to the worker pool
    futures = []
    for audio_path, inst_group in zip(audio_paths, instrument_groups):
        future = CPU_WORKER_POOL.submit(extract_conditioning_worker, audio_path, output_dir, inst_group)
    ...
    for idx, (audio_path, future) in enumerate(futures):
        try:
            result = future.result(timeout=360)  # 6 minute timeout per file
            results[idx] = result
            completed += 1
            print(f"✅ [{completed}/{len(audio_paths)}] Extracted: {Path(audio_path).name}")
        except Exception as e:
            print(f"❌ [{completed+1}/{len(audio_paths)}] Failed: {Path(audio_path).name} - {e}")
            results[idx] = None
            completed += 1

    elapsed = time.time() - start_time
    print(f"🎉 Parallel extraction complete: {completed}/{len(audio_paths)} files in {elapsed:.2f}s")
    print(f"   Average: {elapsed/len(audio_paths):.2f}s per file")
...
def apply_final_audio_processing(audio: torch.Tensor, sample_rate: int = 32000) -> torch.Tensor:
    """
    Apply comprehensive audio processing: HPF, EQ, compression, and limiting.
...
@torch.no_grad()
def generate_batch(
    model: Pipeline, piano_rolls: list, amps: list, rframes: list, rbends: list, encodec_tokens_list: list,
    group, subgroup, steps, seed, adapter_scale, cfg_weight, t0, sr_out,
    instrument_strength=1.0, inst_boost=2.5,
    piano_roll_gain=1.0, amp_gain=1.0, rframe_gain=1.0, rbend_gain=1.0, encodec_gain=1.0,
    use_overlap_decoder=True, original_audio_lengths=None,
    pitch_fidelity_boost=1.0, onset_guidance_boost=2.0, pitch_snap_strength=0.5,
    noise_level=1.0, audio_files=None
):
    """
    Batched GPU generation - process multiple voices simultaneously.

    Args:
        piano_rolls: List of piano roll numpy arrays
        amps: List of amp numpy arrays
        rframes: List of rframe numpy arrays
        rbends: List of rbend numpy arrays
        encodec_tokens_list: List of encodec token tensors
        original_audio_lengths: List of original audio lengths, or None
        audio_files: List of audio file paths for ground truth latents, or None
        ... (other args same as generate())

    Returns:
        list: List of output audio file paths
    """
    batch_size = len(piano_rolls)
    print(f"🚀 Batched GPU generation: Processing {batch_size} voices simultaneously...")
    ...
    # Find maximum length across all tracks for padding
    max_T_slow = max(pr.shape[1] for pr in piano_rolls)
    max_amp_len = max(a.shape[0] for a in amps)
    max_encodec_T = max(et.shape[2] for et in encodec_tokens_list)

    print(f"   Padding tracks to max length: PR={max_T_slow}, amp={max_amp_len}, encodec={max_encodec_T}")

    # Pad all conditioning inputs to the same length before stacking
    def pad_to_length(arr, target_len, axis=-1):
        """Pad numpy array to target length along specified axis"""
    ...
    # Pad piano rolls (128, T)
    padded_prs = [pad_to_length(pr, max_T_slow, axis=1) for pr in piano_rolls]
    # Pad 1D arrays (T,)
    padded_amps = [pad_to_length(a, max_amp_len, axis=0) for a in amps]
    padded_rframes = [pad_to_length(rf, max_amp_len, axis=0) for rf in rframes]
    padded_rbends = [pad_to_length(rb, max_amp_len, axis=0) for rb in rbends]
    # Pad encodec tokens (1, C, T)
    padded_encodec = []
    for et in encodec_tokens_list:
        if et.shape[2] < max_encodec_T:
            pad_len = max_encodec_T - et.shape[2]
            padded_et = torch.nn.functional.pad(et, (0, pad_len), mode='constant', value=0)
        else:
            padded_et = et
        padded_encodec.append(padded_et)

    T_slow = max_T_slow  # Use max length for model

    conds = {
        "piano_roll": torch.stack([torch.from_numpy(pr).float() for pr in padded_prs]).to(device),  # [B,128,T]
        "amp": torch.stack([torch.from_numpy(a).float() for a in padded_amps]).to(device),                  # [B,T]
        "rframe": torch.stack([torch.from_numpy(rf).float() for rf in padded_rframes]).to(device),          # [B,T]
        "rbend": torch.stack([torch.from_numpy(rb).float() for rb in padded_rbends]).to(device),            # [B,T]
        "rbend_mask": torch.stack([torch.from_numpy((rf > 0.5).astype(np.float32)).bool() for rf in padded_rframes]).to(device),
        "encodec_tokens": torch.cat([et.to(device) for et in padded_encodec], dim=0),           # [B,C,Tf]
        "group_id": torch.tensor([gid] * batch_size, dtype=torch.long, device=device),
        "subgroup_id": torch.tensor([sgid] * batch_size, dtype=torch.long, device=device),
    }
    ...
    # Initialize latents - just use pure noise for batch (GT latent mixing is complex for batching)
    x = torch.randn_like(sample_patch.to(device=device, dtype=tokens.dtype))
    print(f"   Initialized latents: {x.shape}")
    ...
    for i in range(steps, 0, -1):
        ...
        # Instrument ON/OFF patches (batched)
        tokens_adapt = tokens.to(dtype=next(model.cond_adapter.parameters()).dtype)
        tokens_on = tokens_adapt.clone(); tokens_on[:, 0, :] *= float(inst_boost)
        tokens_off = tokens_adapt.clone(); tokens_off[:, 0, :].zero_()
        ...
        # Transformer with ControlBranch (batched)
        if ctrl_res_list is not None:
            model._ctrl_residuals = ctrl_res_list
    ...
    # Decode each sample in the batch - PARALLEL DECODING USING THREADING
    print(f"   Decoding {batch_size} audio samples in PARALLEL...")
    ...
    def decode_single_track(idx):
        """Worker function to decode a single track (runs in thread)"""
        try:
            if original_audio_lengths is not None and idx < len(original_audio_lengths):
                audio_len = int(round(original_audio_lengths[idx] * sr_out / DCAE_SR))
            else:
                piano_roll_duration_seconds = T_slow / 43.066
                audio_len = int(round(piano_roll_duration_seconds * sr_out))

            x_single = x[idx:idx+1].to(device=dev, dtype=dtype)
            audio_lengths = torch.tensor([audio_len], dtype=torch.long, device=dev)

            if use_overlap_decoder and hasattr(model.dcae, 'decode_overlap'):
                with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(dev.type=="cuda")):
                    sr_pred, wav_pred = model.dcae.decode_overlap(x_single, audio_lengths=audio_lengths, sr=sr_out)
            else:
                sr_pred, wav_pred = model.dcae.decode(x_single, audio_lengths=audio_lengths, sr=sr_out)

            wav = wav_pred[0].float().cpu()
            wav = apply_final_audio_processing(wav, sample_rate=sr_pred)

            out_dir = Path("./generated_ui"); out_dir.mkdir(exist_ok=True)
            out_path = out_dir / f"{time.strftime('%Y%m%d-%H%M%S')}_batch{idx}_seed{seed}_cfg{cfg_weight:.1f}.wav"
            torchaudio.save(str(out_path), wav, sr_pred)
            output_paths[idx] = str(out_path)
            print(f"      ✓ Track {idx+1}/{batch_size} decoded")
        except Exception as e:
            print(f"      ✗ Track {idx+1}/{batch_size} failed: {e}")
            output_paths[idx] = None

    # Use ThreadPoolExecutor for parallel decoding (CUDA releases GIL)
    max_workers = min(batch_size, 4)  # Limit to 4 concurrent decodings to avoid GPU contention
    print(f"   Using {max_workers} threads for parallel decoding...")

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(decode_single_track, idx) for idx in range(batch_size)]
        # Wait for all to complete
        for future in as_completed(futures):
            future.result()  # This will raise any exceptions that occurred

    # Filter out any None results (failed decodings)
    output_paths = [p for p in output_paths if p is not None]

    print(f"✅ Batched generation complete: {len(output_paths)}/{batch_size} files")
    return output_paths
...
def generate_monophonic_multiple(
    ...
    # Generate voices using batched GPU generation for maximum performance
    voice_outputs = []
    ...
    if AUTO_BATCH_SIZE:
        sample_shape = (8, 16, piano_roll.shape[1])  # Approximate latent shape
        batch_size = estimate_batch_size(model, sample_shape, safety_factor=0.6)
    else:
        batch_size = GPU_BATCH_SIZE

    # Limit batch size to number of voices
    batch_size = min(batch_size, len(voices))
    print(f"🚀 Using GPU batch size: {batch_size} for {len(voices)} voices")

    # Process voices in batches
    num_batches = (len(voices) + batch_size - 1) // batch_size

    for batch_idx in range(num_batches):
        start_idx = batch_idx * batch_size
        end_idx = min(start_idx + batch_size, len(voices))
        batch_voices = voices[start_idx:end_idx]
        current_batch_size = len(batch_voices)

        if progress:
            progress_val = 0.5 + (start_idx / len(voices)) * 0.4
            progress(progress_val, desc=f"Generating batch {batch_idx+1}/{num_batches} ({current_batch_size} voices)...")

        print(f"🎼 Generating batch {batch_idx+1}/{num_batches}: voices {start_idx+1}-{end_idx}")

        # Prepare batched inputs
        piano_rolls_batch = batch_voices  # List of piano rolls
        amps_batch = [amp for _ in range(current_batch_size)]
        rframes_batch = [rframe for _ in range(current_batch_size)]
        rbends_batch = [rbend for _ in range(current_batch_size)]
        encodec_tokens_batch = [encodec_tokens for _ in range(current_batch_size)]
        audio_lengths_batch = [original_audio_length] * current_batch_size if original_audio_length else None
        audio_files_batch = [audio_file] * current_batch_size if audio_file else None

        # Use different seeds for variety (same as sequential approach)
        batch_seed = base_seed + start_idx * 1000

        # Call batched generation
        batch_outputs = generate_batch(
            model, piano_rolls_batch, amps_batch, rframes_batch, rbends_batch, encodec_tokens_batch,
            group, subgroup, steps, batch_seed, adapter_scale, cfg_weight, t0, sr_out,
            instrument_strength, inst_boost, piano_roll_gain, amp_gain, rframe_gain, rbend_gain, encodec_gain,
            use_overlap_decoder, audio_lengths_batch, pitch_fidelity_boost, onset_guidance_boost, pitch_snap_strength,
            noise_level, audio_files_batch
        )

        # Rename outputs to include voice numbers
        for idx_in_batch, batch_output in enumerate(batch_outputs):
            global_voice_idx = start_idx + idx_in_batch
            voice_seed = base_seed + global_voice_idx * 1000
            voice_path = out_dir / f"{timestamp}_voice{global_voice_idx+1}_seed{voice_seed}_cfg{cfg_weight:.1f}.wav"
            shutil.move(batch_output, str(voice_path))
            voice_outputs.append(str(voice_path))
            print(f"✅ Voice {global_voice_idx+1} saved: {voice_path.name}")

            # Notify callback
            if voice_complete_callback:
                voice_complete_callback(global_voice_idx, str(voice_path), len(voices))
...
def init_worker(**kwargs):
    ...
    # Initialize CPU worker pool for parallel processing
    cpu_count = mp.cpu_count()
    num_workers = max(1, cpu_count - 1)  # Leave one core for main process
    print(f"🔧 Initializing CPU worker pool in Celery worker ({num_workers} workers from {cpu_count} cores)...")
    init_cpu_worker_pool(max_workers=num_workers)
    print(f"✅ CPU worker pool initialized with {num_workers} workers")

@worker_process_shutdown.connect
def shutdown_worker(**kwargs):
    """Shutdown worker pool when Celery worker stops"""
    print(f"🛑 Shutting down CPU worker pool in Celery worker...")
    shutdown_cpu_worker_pool()
    print(f"✅ CPU worker pool shut down")
    ...
    try:
        ...
        if is_midi_file(audio_file_path) and monophonic_mode and not enable_voice_separation:
            ...
            if is_multi:
                ...
                # Step 2: Render each track with FluidSynth (using correct soundfont) - PARALLEL
                print(f"\n🎹 Step 2: Rendering each track with FluidSynth ({subgroup} soundfont)...")
                instrument_groups = [subgroup] * len(track_midi_files)
                track_audio_files = render_midi_batch_parallel(
                    track_midi_files,
                ...
                # Step 2.5: Real fatten mode - transpose MIDI and render octave-up versions - PARALLEL
                if fatten_mode and fatten_type == "real":
                    print(f"\n🎚️ Step 2.5 REAL FATTEN MODE: Creating octave-up MIDI and rendering...")
                    original_track_count = len(track_midi_files)
                    octave_midi_files = []

                    # First, create all octave-up MIDI files
                    for i, track_midi in enumerate(track_midi_files):
                    ...
                    # Now render all octave-up versions in parallel
                    print(f"   Rendering {len(octave_midi_files)} octave-up tracks in parallel...")
                    octave_instrument_groups = [subgroup] * len(octave_midi_files)
                    octave_audio_files = render_midi_batch_parallel(
                        octave_midi_files,
                        output_dir=str(debug_dir),
                        instrument_groups=octave_instrument_groups,
                        use_parallel=True
                    )
                ...
                # Step 3: Process tracks SEQUENTIALLY through full pipeline (extract → generate → decode)
                # Each track completes fully and returns before moving to next track
                print(f"\n🚀 Step 3: Processing {len(track_audio_files)} tracks SEQUENTIALLY (streaming results)...")
                print(f"   Pipeline: Extract Conditioning → Generate → Decode → Post-Process → Return")
                print(f"   This ensures maximum efficiency with GPU (one track at a time, no batching overhead)")

                import torchaudio

                # Process each track completely before moving to next
                for track_idx, track_audio in enumerate(track_audio_files):
                    try:
                        track_name = Path(track_audio).name
                        print(f"\n{'='*60}")
                        print(f"🎵 Track {track_idx + 1}/{len(track_audio_files)}: {track_name}")
                        print(f"{'='*60}")

                        # STEP 1: Extract conditioning for this track
                        print(f"   [1/3] Extracting conditioning...")
                        extraction = extract_conditioning_from_audio(
                            track_audio,
                            output_dir="./extracted_conditioning",
                            instrument_group=subgroup
                        )
                        print(f"   ✅ Conditioning extracted")

                        # STEP 2: Load conditioning and generate audio
                        print(f"   [2/3] Loading conditioning and generating audio...")
                        wav, sr = torchaudio.load(track_audio)
                        actual_duration = wav.shape[-1] / sr
                        window_slow = int(actual_duration * fps)

                        piano_roll, amp, rframe, rbend, encodec_tokens = load_conditioning(extraction, window_slow)
                        print(f"   Conditioning loaded: PR={piano_roll.shape}, amp={amp.shape}, duration={actual_duration:.2f}s")
                        ...
                        print(f"   ✅ Generation complete: {Path(voice_output).name}")

                        # STEP 3: Post-process and save
                        print(f"   [3/3] Post-processing and saving...")

                        # Speed up this voice immediately if we slowed it down
                        if tape_speed < 1.0:
                            print(f"   🎞️ Restoring speed ({1.0/tape_speed:.2f}x)...")
                            speedup_factor = 1.0 / tape_speed
                            ...
                            print(f"   ✅ Speed restored to original")
                        ...
                        completed_voices.append(download_url)
                        print(f"   ✅ Saved as output {output_track_counter}: {voice_output_path.name}")
                        ...
                        print(f"   ✅ Track {track_idx + 1} COMPLETE and returned to frontend!\n")

                    except Exception as e:
                        print(f"   ❌ Track {track_idx + 1} FAILED: {e}")
                        import traceback
                        traceback.print_exc()
                        # Continue to next track even if this one fails
...
async def generate_audio(
    ...
    use_overlap_decoder = True  # Default value
    if params:
...
@app.get("/api/get-midi-info/{filename}")
async def get_midi_info_endpoint(filename: str):
    """Get metadata/track info for a specific MIDI file"""
    from preprocess_midi_metadata import get_midi_info
    ...
    try:
        metadata = get_midi_info(str(file_path))
        print(f"📊 Serving MIDI info for: {filename}")
        return metadata
    except Exception as e:
        print(f"❌ Error getting MIDI info for {filename}: {e}")
...
def main():
    ...
    cpu_count = mp.cpu_count()
    num_workers = max(1, cpu_count - 1)  # Leave one core for main process
    print(f"--- Initializing CPU worker pool ({num_workers} workers from {cpu_count} cores) ---")
    init_cpu_worker_pool(max_workers=num_workers)
    ...
    # Cleanup on exit
    import atexit
    atexit.register(shutdown_cpu_worker_pool)
...
# FastAPI should be run with: uvicorn genfrominterface_parallel:app --host 0.0.0.0 --port 8000
# Celery worker should be run with: celery -A genfrominterface_parallel worker --loglevel=info
# Gradio UI (parallel version) - run with python:

if __name__ == "__main__":
    # Required for multiprocessing on some platforms (especially Windows)
