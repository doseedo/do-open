...
#TRAIN FROM SCRATCH
        ...
        if self.use_ctrl_branch:
            ...
            # matches the saved checkpoint params, avoids unexpected keys
            self._ctrl_proj = nn.ModuleList([])
            for _ in tail_blocks:
                conv = nn.Conv1d(C_latent, C_latent, 1)
                nn.init.zeros_(conv.weight)  # keep zero - gradients flow through identity _ctrl_zero
            ...
            def make_inject(idx):
                def hook(module, inp, out):
                    ...
                    def add_to(x, add, idx):
                        ...
                        # --- channel align with prebuilt projection conv ---
                        add_1d = self._ctrl_proj[idx](add_1d)                # [B,C_latent,T]
                        add_1d = self._ctrl_zero[idx](add_1d)                # identity-initialized final projection
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        if hasattr(self, "_ctrl_proj") and self._ctrl_proj is not None:
            ...
            if ctrl_proj_keys:
                # Replace Identity layers with Conv1d layers matching checkpoint shapes
                for i, key in enumerate(sorted(ctrl_proj_keys)):
                    w = sd.get(key)
                    if w is not None and i < len(self._ctrl_proj):
                        out_ch, in_ch = int(w.shape[0]), int(w.shape[1])
                        self._ctrl_proj[i] = nn.Conv1d(in_ch, out_ch, kernel_size=1)
                        # Initialize to zero like the original lazy creation
                        nn.init.zeros_(self._ctrl_proj[i].weight)
                        if self._ctrl_proj[i].bias is not None:
                            nn.init.zeros_(self._ctrl_proj[i].bias)
                print(f"[resume] reconstructed _ctrl_proj with shapes from checkpoint")
