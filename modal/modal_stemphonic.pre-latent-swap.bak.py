...
image = (
    ...
    #
    # audio-separator is also commented out in the requirements file itself
    # (line 14) and installed separately below to make the intention obvious.
    .pip_install_from_requirements(
        "/home/arlo/do2/stemphonic_requirements_modal.txt",
        extra_options="--no-deps",
    )
    # audio-separator — pinned version, no-deps (all transitives already
    # satisfied by the requirements pass above).
...
@app.cls(
    ...
    min_containers=0,          # scale to zero — cost-critical
...
class Stemphonic:
    ...
    def setup_cpu_state(self):
        ...
        for ephemeral in [
            ...
            "/scratch/audio_separator_out",
        ]:
            pathlib.Path(ephemeral).mkdir(parents=True, exist_ok=True)

        # ---- sys.path mirrors stemphonic_server.py:29-31 ----
