...
app = modal.App("doseedo-stemphonic-staging")
...
@app.cls(
    ...
    secrets=[
        ...
        modal.Secret.from_name("doseedo-gate-staging"),
    ],
    timeout=60 * 30,           # 30 min per request (long ACE-Step gens)
    scaledown_window=60 * 5,   # 5 min warm window (staging — cheap)
    min_containers=0,          # scale to zero between staging sessions
    max_containers=1,
