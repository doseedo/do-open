...
export function useGeneration() {
    ...
    for (const voiceFile of completed_voices) {
      ...
      const track = {
        ...
        metadata: {
          ...
          // Trumpet mute: defaults for partial results (no dry_file_paths yet)
          isMuted: false,
  ...
  const handleFinalResults = useCallback(async (result, busId, params, placeholderTrackId) => {
    ...
    const { file_paths, input_files, dry_file_paths } = result;
    ...
    for (const filePath of file_paths) {
      ...
      // Determine if this track was auto-muted during generation
      const isMutedTrack = params.instrumentSubgroup === 'muted_trumpet' && !!dry_file_paths;
      const dryUrl = dry_file_paths?.[file_paths.indexOf(filePath)] || null;
      ...
      const track = {
        ...
        metadata: {
          ...
          tempo: null, // Will be extracted from MIDI file if available
          // Trumpet mute: store both dry and muted URLs for toggling
          isMuted: isMutedTrack,
          originalAudioUrl: dryUrl,
          mutedAudioUrl: isMutedTrack ? filePath : null,
        },
        instrumentGroup: params.instrumentGroup,
        instrumentSubgroup: params.instrumentSubgroup
      };
