...
// Global in-memory cache for audio buffers (persists across component mounts)
    ...
    const loadAudio = async () => {
      try {
        ...
        if (audioBufferCache.has(audioUrl)) {
          console.log(`🔄 Using cached audio buffer for: ${audioUrl}`);
        ...
        // Not in cache, fetch and decode
        console.log(`📥 Fetching and caching audio buffer for: ${audioUrl}`);
        ...
        if (audioBufferCache.size > MAX_CACHE_SIZE) {
          ...
          console.log(`🗑️ Removed oldest cached buffer: ${firstKey}`);
        }
...
function renderWaveform(ctx, audioBuffer, width, height, color, cropStart = 0, cropEnd = 0) {
  ...
  for (let i = 0; i < numBars; i++) {
    ...
    const barHeight = Math.max(2, rms * normalizationFactor * 2.5);
