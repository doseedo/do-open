...
const AudioLabeler = () => {
  ...
  const [drumOnsetStem, setDrumOnsetStem] = useState('original');

  // Reset stem selection when navigating drum onsets
  useEffect(() => { setDrumOnsetStem('original'); }, [currentIndex, dataSource]);
      ...
      {activeTab === 'midi' && (
        <>
          ...
          <MidiCorrectionGrid
            ...
            isDrumMode={dataSource === 'drum_onsets' || currentItem?.group === 'drums' || currentItem?.group === 'e-drums' || currentItem?.group === 'percussion'}
