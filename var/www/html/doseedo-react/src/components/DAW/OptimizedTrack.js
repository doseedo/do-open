...
const OptimizedTrack = React.memo(({ track, busId, index, isExpanded, isSelected, isMasterView = false, trackHeight = 72 }) => {
  ...
  return (
    <div
      ...
      className={`${styles.track} ${isSelected ? styles.selected : ''} ${isDragging ? styles.dragging : ''} ${!isLoaded || track.isPlaceholder ? styles.trackLoading : ''} ${isInInpaintMode ? styles.inpaintMode : ''} ${track.isMuted ? styles.muted : ''} ${track.isPlaceholder ? styles.placeholder : ''}`}
      style={{
