...
class TunaFXService {
   ...
   */
  getFXBusInput() {
    if (!this.initialized || !this.fxBusInput) {
      console.warn('🎛️ Tuna FX not initialized, cannot get input');
      return null;
    }
   ...
   */
  getFXBusOutput() {
    if (!this.initialized || !this.fxBusOutput) {
      console.warn('🎛️ Tuna FX not initialized, cannot get output');
      return null;
    }
  ...
  destroy() {
    if (this.fxBusInput) {
      try { this.fxBusInput.disconnect(); } catch (e) { /* ignore */ }
      this.fxBusInput = null;
    }
    if (this.fxBusOutput) {
      try { this.fxBusOutput.disconnect(); } catch (e) { /* ignore */ }
      this.fxBusOutput = null;
    }

    // Disconnect all effects and null them
    Object.keys(this.effects).forEach(key => {
      if (this.effects[key]) {
        try { this.effects[key].disconnect(); } catch (e) { /* ignore */ }
