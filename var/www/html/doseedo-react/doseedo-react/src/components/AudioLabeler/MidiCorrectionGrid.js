...
const MidiCorrectionGrid = ({ audioPath, isDrumMode, onSave, currentTime = 0, duration = 0, isPlaying = false }) => {
  ...
  const noteRange = useMemo(() => {
    ...
    return { min, max };
  }, [notes, originalNotes, refinedNotes, effectiveDrumMode]);
  ...
  useEffect(() => {
    ...
    drawNotes(ctx);
    drawPlayhead(ctx);
  }, [drawGrid, drawGhostNotes, drawCorrectedOverlay, drawRefinedOverlay, drawNotes, drawPlayhead]);
    ...
    <div className="midi-correction-grid">
      <div className="midi-grid-toolbar">
        ...
        <div className="midi-grid-toggles">
          ...
          {effectiveDrumMode && <span className="midi-drum-badge">DRUM MODE</span>}
        </div>
