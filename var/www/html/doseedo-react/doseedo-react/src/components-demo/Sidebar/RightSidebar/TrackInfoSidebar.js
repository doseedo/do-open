 ...
 */
const TrackInfoSidebar = React.memo(({ embedded = false }) => {
  const { state, dispatch } = useApp();

  const isCollapsed = embedded ? false : (state.stemsSidebar?.isCollapsed ?? true);
  ...
  const [showFX, setShowFX] = useState(true); // Default expanded
  ...
  const [activeTab, setActiveTab] = useState('track'); // 'track' or 'fx'
  const [trumpetMuted, setTrumpetMuted] = useState(false); // For 3D trumpet icon toggle
  ...
  return (
    <div
      className={`${styles.sidebar} ${isCollapsed ? styles.collapsed : ''} ${embedded ? styles.embedded : ''}`}
      style={embedded ? { position: 'relative', transform: 'none', width: '100%', height: '100%' } : undefined}
    >
      {/* Toggle Button - hide in embedded mode */}
      {!embedded && (
      ...
      <div className={styles.content}>
        ...
        <div className={styles.header}>
          <div className={styles.tabContainer}>
            ...
            >
              <i className="fa-solid fa-music"></i> Track
            </button>
            <button
              className={`${styles.tab} ${activeTab === 'fx' ? styles.tabActive : ''}`}
              onClick={() => setActiveTab('fx')}
            >
              <i className="fa-solid fa-sliders"></i> FX
            </button>
          </div>
        ...
        {/* FX Panel - shown when FX tab is active */}
        {activeTab === 'fx' && (
          <div className={styles.fxTabContent}>
            {/* Reverb */}
            <div className={styles.fxSection}>
              <div className={styles.fxSectionHeader}>
              ...
              <div className={styles.fxControl}>
                <label>
                  ...
                  />
                  <span className={styles.fxValue}>{(state.reverbDecay || 2.5).toFixed(1)}s</span>
                </label>
              </div>
              <div className={styles.fxControl}>
                <label>
                  ...
                  />
                  <span className={styles.fxValue}>{Math.round((state.reverbRoomSize || 0.5) * 100)}%</span>
                </label>
              </div>
              <div className={styles.fxControl}>
                <label>
                  ...
                  />
                  <span className={styles.fxValue}>{Math.round((state.reverbDamping || 0.5) * 100)}%</span>
                </label>
              </div>
            </div>
            ...
            <div className={styles.fxSection}>
              ...
              <div className={styles.fxControl}>
                <label>
                  ...
                  />
                  <span className={styles.fxValue}>{(state.filterResonance || 1).toFixed(1)}Q</span>
                </label>
              </div>
            </div>
        ...
        {/* Track Info Display - shown when Track tab is active */}
        {activeTab === 'track' && <div className={styles.infoDisplay}>
          {!selectedTrack && !selectedBus ? (
            ...
            <div className={styles.infoContent}>
              ...
              {selectedTrack && (
                <div className={styles.instrument3DSection}>
                  {(() => {
                    ...
                    // Check for drums
                    const isDrums = instrumentGroup === 'drums' ||
                      instrumentSubgroup === 'drums' ||
                      instrumentSubgroup === 'electronic' ||
                      instrumentSubgroup === 'jazz' ||
                      instrumentSubgroup === 'orchestral' ||
                      trackName.includes('drum') ||
                      trackName.includes('percussion') ||
                      trackName.includes('beat');
                          ...
                          <div className={styles.instrument3DControls}>
                            <label className={styles.muteToggle}>
                              <input
                                ...
                                onChange={(e) => setTrumpetMuted(e.target.checked)}
                              />
                              <span>Muted</span>
                            </label>
                          </div>
              ...
              <div className={styles.section}>
                <h4>
                  <i className="fa-solid fa-file-audio"></i> Source File
                </h4>
                <div className={styles.item}>
                  <span className={styles.value} style={{ wordBreak: 'break-all', fontSize: '11px' }}>
                    {selectedTrack.audioUrl || '-'}
                  </span>
                </div>
              </div>
              ...
              <div className={styles.section}>
                ...
                >
                  {state.inpaintMode?.enabled && state.inpaintMode?.trackId === selectedTrack.id ? (
                    <>
                      <i className="fa-solid fa-times"></i> Cancel Inpaint
                    </>
                  ) : (
                    <>
