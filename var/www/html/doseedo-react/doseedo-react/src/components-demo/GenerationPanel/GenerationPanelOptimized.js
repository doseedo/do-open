...
const InstrumentSelection = React.memo(({ params, updateParam, selectedGenre }) => {
  ...
  // Genre to instruments mapping (subgroups for each genre)
  const genreInstruments = useMemo(() => ({
    cinematic: [
    ...
    electronic: [
      { name: 'Synth Lead', group: 'keys', subgroup: 'keys', icon: '/assets/icons/keyboard.png' },
      { name: 'Bass', group: 'bass', subgroup: 'electric_bass', icon: '/assets/icons/elecbass.png' },
      { name: 'Drums', group: 'drums', subgroup: 'electronic', icon: '/assets/icons/elecdrums.png' },
    ...
    jazz: [
      { name: 'Saxophone', group: 'winds', subgroup: 'sax', icon: '/assets/icons/sax.png' },
      { name: 'Piano', group: 'piano', subgroup: 'acoustic_piano', icon: '/assets/icons/piano.png' },
      { name: 'Upright Bass', group: 'bass', subgroup: 'upright_bass', icon: '/assets/icons/elecbass.png' },
      { name: 'Drums', group: 'drums', subgroup: 'jazz', icon: '/assets/icons/drumkit.png' },
      { name: 'Trumpet', group: 'brass', subgroup: 'trumpet', icon: '/assets/icons/tpt.png' },
      { name: 'Trombone', group: 'brass', subgroup: 'trombone', icon: '/assets/icons/tbn.png' }
    ],
    hiphop: [
  ...
  // Clicking active group again deselects it
  const handleGroupChange = useCallback((newGroup) => {
    if (params.instrumentGroup === newGroup) {
      // Toggle off - deselect instrument
      updateParam('instrumentGroup', null);
      updateParam('instrumentSubgroup', null);
    } else {
      // Select new group
  ...
  }, [updateParam, instrumentSubgroups, params.instrumentGroup]);
  ...
  // Handle genre instrument selection - toggle off if clicking active
  const handleGenreInstrumentSelect = useCallback((instrument) => {
    if (params.instrumentGroup === instrument.group && params.instrumentSubgroup === instrument.subgroup) {
      // Toggle off
      updateParam('instrumentGroup', null);
      updateParam('instrumentSubgroup', null);
    } else {
      // Select this instrument
      updateParam('instrumentGroup', instrument.group);
      updateParam('instrumentSubgroup', instrument.subgroup);
    }
  }, [params.instrumentGroup, params.instrumentSubgroup, updateParam]);

  // Get genre instruments if genre is selected
  const genreSubgroups = selectedGenre ? genreInstruments[selectedGenre] : null;

  return (
    <>
      {/* When genre is selected, show only genre subgroups */}
      {selectedGenre && genreSubgroups ? (
        <>
          <div className={styles.instrumentSubgroupGrid}>
            {genreSubgroups.map(instrument => (
              <button
                key={`${instrument.group}-${instrument.subgroup}`}
                type="button"
                className={`${styles.instrumentSubgroupBtn} ${
                  params.instrumentGroup === instrument.group && params.instrumentSubgroup === instrument.subgroup
                    ? styles.active
                    : ''
                }`}
                onClick={() => handleGenreInstrumentSelect(instrument)}
              >
                <img
                  src={instrument.icon}
                  alt={instrument.name}
        ...
        <>
          {/* Instrument Group Grid - Only show when no genre selected */}
...
const GenreSelection = React.memo(({ selectedGenre, onGenreSelect, selectedFormSection, onGenerateForSection }) => {
  const genres = useMemo(() => [
    { id: 'cinematic', label: 'Cinematic', icon: 'fa-film' },
    { id: 'jazz', label: 'Jazz', icon: 'fa-saxophone' },
    { id: 'reggae', label: 'Reggae', icon: 'fa-leaf' },
    { id: 'electronic', label: 'Electronic', icon: 'fa-bolt' },
    { id: 'orchestral', label: 'Orchestral', icon: 'fa-music' },
    { id: 'ambient', label: 'Ambient', icon: 'fa-cloud' },
    { id: 'rock', label: 'Rock', icon: 'fa-guitar' },
    { id: 'hiphop', label: 'Hip Hop', icon: 'fa-microphone' },
    { id: 'world', label: 'World', icon: 'fa-globe' }
  ], []);

  // Handle genre click - if form section selected, trigger generation
  const handleGenreClick = useCallback((genreId) => {
    if (selectedGenre === genreId) {
      onGenreSelect(null); // Deselect
    } else {
      onGenreSelect(genreId);
      // If a form section is selected, trigger generation for that section
      if (selectedFormSection && onGenerateForSection) {
        onGenerateForSection(genreId, selectedFormSection);
      }
    }
  }, [selectedGenre, onGenreSelect, selectedFormSection, onGenerateForSection]);

  return (
    <div className={styles.genreScrollContainer}>
      <div className={styles.genreRow}>
        {genres.map(genre => (
          <button
            key={genre.id}
            className={`${styles.genreChip} ${selectedGenre === genre.id ? styles.selected : ''}`}
            data-genre={genre.id}
...
GenreSelection.displayName = 'GenreSelection';
...
const GenerationPanelOptimized = React.memo(() => {
  ...
  // selectedGenre is now from context state (state.selectedGenre)
    ...
    <div id="audiodiv" className={styles.panelGrid}>
      ...
      <div className={styles.section}>
        ...
        <GenreSelection
          ...
          onGenreSelect={(genre) => dispatch({ type: 'SET_SELECTED_GENRE', payload: genre })}
          selectedFormSection={state.selectedFormSection}
          onGenerateForSection={(genre, section) => {
            dispatch({ type: 'GENERATE_FOR_FORM_SECTION', payload: { genre, section } });
          }}
        />
      </div>
