 ...
 */
const OptimizedTrack = React.memo(({ track, busId, index, isExpanded, isSelected, isMasterView = false, trackHeight = 72, enablePointerEvents = false }) => {
  ...
  const initiatePluginDrag = useCallback(() => {
    ...
    const fileName = `${track.name || 'audio'}.mp3`;

    // Check if we're in a JUCE webview (it exposes window.juce or similar)
    if (window.juce && window.juce.startDragFile) {
      // JUCE bridge method to initiate native drag
      window.juce.startDragFile(track.audioUrl, fileName);
      console.log('🎵 JUCE drag initiated:', fileName);
    } else if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.dragFile) {
      // WebKit (macOS) message handler
      window.webkit.messageHandlers.dragFile.postMessage({
        url: track.audioUrl,
        fileName: fileName,
        trackId: track.id
      });
      console.log('🎵 WebKit drag initiated:', fileName);
      ...
      console.log('🎵 WebView2 drag initiated:', fileName);
    } else {
      // Fallback: trigger download if no plugin bridge available
      console.log('⚠️ No plugin bridge detected, falling back to download');
  ...
  }, [track.audioUrl, track.name, track.id]);
  ...
  // Get waveform color - prefer track's stored color, then CSS variable
  const [waveformColor, setWaveformColor] = useState(track.waveformColor || '#667eea');
  const [isHovering, setIsHovering] = useState(false);

  // Update waveform color when track's stored color changes
  useEffect(() => {
    if (track.waveformColor) {
      setWaveformColor(track.waveformColor);
    }
  }, [track.waveformColor]);

  // Update waveform color when selection or hover changes (only if no stored color)
  useEffect(() => {
    if (!track.waveformColor && trackRef.current) {
  ...
  }, [isSelected, isHovering, track.waveformColor]);

  // Reset hover state when shift is pressed
  useEffect(() => {
    if (state.shiftHeld && isHovering) {
      setIsHovering(false);
    }
  }, [state.shiftHeld, isHovering]);
  ...
  const handleSelect = useCallback((e) => {
    ...
    // Skip selection when shift is held (marquee replace mode)
  ...
  }, [dispatch, track.id, isDragging, isResizing, state.stemsSidebar, state.shiftHeld]);
  ...
  const handleMouseDragStart = useCallback((e) => {
    ...
    if (state.shiftHeld) return; // Skip drag when shift is held (allow marquee replace)
  ...
  }, [track.id, track.startPosition, track.cropStart, track.width, busId, dispatch, pixelsPerSecond, snapToGrid, isMasterView, state.pluginMode, track.audioUrl, initiatePluginDrag, state.shiftHeld]);
  ...
  return (
    <div
      ...
      onMouseEnter={() => !state.shiftHeld && setIsHovering(true)}
      onMouseLeave={() => setIsHovering(false)}
    >
      {/* Placeholder loading indicator - show when placeholder OR loading */}
      {(track.isPlaceholder || track.isLoading) ? (
        <PlaceholderWaveform
          width={trackWidth}
          height={trackHeight}
