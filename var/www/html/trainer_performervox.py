...
# OG ACE STEP DIFFUSION
    ...
    def training_step(self, batch, batch_idx):
        ...
        reference_latent = batch.get("reference_latent")  # [B, 8, 16] or None
        ...
        if self.use_ctrl_branch:
                        ...
                        if vc is not None and "lyrics_tensors" in vc:
                            ...
                            if "phoneme_embeddings" in lyrics_tensors:
                                ...
                                if isinstance(token_ids, torch.Tensor):
                                    ...
                                    lyric_embs = self.lyric_token_embedder(token_ids.long().to(x_noisy.device))
            ...
            # CRITICAL FIX: Interpolate lyric embeddings to match T_slow (frame-aligned)
            if lyric_embeddings is not None:
                ...
                # Interpolate sequence-aligned embeddings to frame-aligned
                if L != T_slow:
                    # Transpose to [B, 256, L] for interpolation
                    lyric_embs_interp = lyric_embeddings.transpose(1, 2)  # [B, 256, L]
                    lyric_embs_interp = F.interpolate(lyric_embs_interp, size=T_slow, mode='linear', align_corners=False)
                    lyric_embeddings = lyric_embs_interp.transpose(1, 2)  # [B, T_slow, 256]
