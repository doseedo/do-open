# ~/Data/ACE-Step/acestep/pipeline_ace_step.py
# Apache 2.0
...
# from acestep.models.ace_step_transformer import PerformerTransformer as ACEStepTransformer2DModel
...
class ACEStepTrainComponents:
    """
    Minimal, training-first loader:
    ...
    def __init__(self, checkpoint_dir: str, device_id: int = 0, dtype: str = "float32"):
        ...
        logger.info(f"[ACEStepTrainComponents] root = {self.root}")
        logger.info(f"[ACEStepTrainComponents] device = {self.device}, dtype = {self.dtype}")
    ...
    def load_dcae(self):
        ...
        self.music_dcae = m
        logger.info("[ACEStepTrainComponents] MusicDCAE loaded (frozen).")
        return self.music_dcae
   
    def _get_transformer_class(self, from_scratch=False):
        if from_scratch:
            from acestep.models.ace_step_transformer_CUSTOM import PerformerTransformer as CustomTransformer
            return CustomTransformer
        else:
            from acestep.models.ace_step_transformer import ACEStepTransformer2DModel as OfficialTransformer
    ...
    def build_transformer_pretrained(self):
        ...
        logger.info("[ACEStepTrainComponents] Transformer loaded WITH PRE-TRAINED weights.")
    ...
    def build_transformer_random(self):
        ...
        logger.info("[ACEStepTrainComponents] Built custom transformer via from_config().")

        model = model.to(self.device).to(self.dtype).train()
        self.transformer = model
        logger.info("[ACEStepTrainComponents] Transformer initialized RANDOMLY (no weights loaded).")
        return self.transformer