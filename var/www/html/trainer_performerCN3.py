...
try:
    ...
    _HAS_EMA = False
    print("Warning: torch-ema not found. Install with: pip install torch-ema")

# EMA helper for temporary weight swapping
class _Swap:
    def __init__(self, ema, module):
        self.ema, self.module = ema, module
    def __enter__(self):
...
class Pipeline(LightningModule):
    def __init__(
        ...
        self.hparams.preview_seconds = getattr(self.hparams, "preview_seconds", 10)  # 2–3s is plenty
...
#TRAIN FROM SCRATCH
        ...
        # Initialize EMA after transformers is ready
        if _HAS_EMA:
            self._ema = ExponentialMovingAverage(
                list(self.transformers.parameters()),  # Track ALL params, even frozen ones
                decay=0.999,
                use_num_updates=True  # Bias correction for warmup/accumulation
            )
            # Keep EMA on GPU for best performance (uses more VRAM but faster)
            self._ema_enabled = True
            total_params = len(list(self.transformers.parameters()))
            trainable_params = sum(1 for p in self.transformers.parameters() if p.requires_grad)
            print(f"[EMA] Initialized with decay=0.999 on {total_params} total params ({trainable_params} currently trainable)")
        else:
            self._ema_enabled = False
            print("[EMA] Disabled - torch-ema not available")
        ...
        # Initialize subgroup ID alignment check (defer to fit_start when dataset is available)
        self._subgroup_id_remap = None
    ...
    def _setup_class_weights(self, manifest_json):
        ...
        try:
            ...
            # Convert to hours and compute softer inverse frequency weights
            alpha = 0.5  # 0=no weight, 1=full inverse frequency
            floor_weight = 0.6  # Minimum weight for majority classes

            # Group weights with floor
            group_hours = {g: max(1e-6, s/3600.0) for g, s in group_dur.items()}
            group_weights_raw = {g: (1.0 / h) ** alpha for g, h in group_hours.items()}
            mean_weight = sum(group_weights_raw.values()) / len(group_weights_raw)
            group_weights_normed = {g: max(floor_weight, w/mean_weight) for g, w in group_weights_raw.items()}

            for name, weight in group_weights_normed.items():
                gid = self.group2id.get(name, None)
                if gid is not None:
                    self.group_ce_w[gid] = weight

            # Subgroup weights with floor
            sub_hours = {s: max(1e-6, d/3600.0) for s, d in sub_dur.items()}
            sub_weights_raw = {s: (1.0 / h) ** alpha for s, h in sub_hours.items()}
            mean_sub_weight = sum(sub_weights_raw.values()) / len(sub_weights_raw)
            sub_weights_normed = {s: max(floor_weight, w/mean_sub_weight) for s, w in sub_weights_raw.items()}

            for name, weight in sub_weights_normed.items():
                sid = self.subgroup2id.get(name, None)
                if sid is not None:
                    self.sub_ce_w[sid] = weight

            print(f"[balance] Class weights setup with alpha={alpha}, floor={floor_weight}")
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        if getattr(self, "_ema_enabled", False) and "ema_state" in checkpoint:
            try:
                self._ema.load_state_dict(checkpoint["ema_state"])
                print("[EMA] Restored state from checkpoint")
            except Exception as e:
    ...
    def _safe_remap(self, x, remap_vec):
        """Safely remap tensor values with bounds checking, fallback to 0 for out-of-bounds."""
        if remap_vec is None:
            return x
        ok = x < remap_vec.numel()
        out = torch.zeros_like(x)
        out[ok] = remap_vec[x[ok]]
        # out[~ok] stays 0 for any out-of-bounds indices
        return out

    def _id_alignment_check(self, ds):
        """Critical: Ensure subgroup IDs from dataset match our internal mapping."""
        try:
            print("[align] Checking subgroup ID alignment...")

            # Try to get dataset's ID to name mapping
            ds_id2name = []
            sample_items = []

            # Sample items to understand the dataset's subgroup structure
            max_samples = min(len(ds), 500)  # Sample enough to see all subgroups
            seen_subgroups = set()

            for i in range(max_samples):
                try:
                    # Get metadata without loading full audio
                    if hasattr(ds, 'get_meta'):
                        meta = ds.get_meta(i)
                    elif hasattr(ds, 'items_meta') and ds.items_meta and i < len(ds.items_meta):
                        meta = ds.items_meta[i]
                    else:
                        # Fallback: load one item to get structure
                        item = ds[i]
                        if isinstance(item, dict) and "instrument" in item:
                            subgroup_id = item["instrument"]["subgroup_id"].item() if hasattr(item["instrument"]["subgroup_id"], "item") else item["instrument"]["subgroup_id"]
                            # Try to infer name from manifest
                            import json
                            with open(self.manifest_json, "r") as f:
                                manifest_data = json.load(f)
                            if i < len(manifest_data):
                                meta = manifest_data[i]
                            else:
                                continue
                        else:
                            continue

                    sub_name = (meta.get("sub_group") or meta.get("subgroup") or "unknown").lower()
                    if hasattr(ds, 'items') and i < len(ds.items):
                        subgroup_id = ds.items[i].get("subgroup_id", 0)
                    else:
                        # Try to load one item to get the subgroup_id
                        item = ds[i]
                        if isinstance(item, dict) and "instrument" in item:
                            subgroup_id = item["instrument"]["subgroup_id"]
                            if hasattr(subgroup_id, "item"):
                                subgroup_id = subgroup_id.item()
                        else:
                            continue

                    if sub_name not in seen_subgroups:
                        seen_subgroups.add(sub_name)
                        sample_items.append((subgroup_id, sub_name))

                except Exception as e:
                    print(f"[align] Warning: Could not process item {i}: {e}")
                    continue

            # Sort by subgroup_id to build the mapping
            sample_items.sort(key=lambda x: x[0])
            ds_id2name = [name for _, name in sample_items]

            print(f"[align] Dataset subgroups found: {ds_id2name}")
            print(f"[align] Our subgroup mapping: {sorted(self.subgroup2id.keys())}")

            # Build remapping table
            remap = {}
            mismatches = []

            for ds_sid, name in enumerate(ds_id2name):
                if name in self.subgroup2id:
                    our_sid = self.subgroup2id[name]
                    remap[ds_sid] = our_sid
                    if ds_sid != our_sid:
                        mismatches.append(f"{name}: ds_id={ds_sid} -> our_id={our_sid}")
                else:
                    print(f"[align] WARNING: Dataset subgroup '{name}' not in our mapping!")
                    remap[ds_sid] = 0  # Map to first subgroup as fallback

            if mismatches:
                print(f"[align] ⚠️  ID MISMATCHES DETECTED! Applying remap:")
                for mismatch in mismatches:
                    print(f"  {mismatch}")

                # Create safe remap tensor with proper bounds checking
                max_id = max(remap.keys()) if remap else 0
                remap_vec = torch.zeros(max_id + 1, dtype=torch.long, device=self.device)  # default 0
                for k, v in remap.items():
                    if k <= max_id:
                        remap_vec[k] = v
                self._subgroup_id_remap = remap_vec
                print(f"[align] ✅ Safe subgroup ID remap tensor created: {self._subgroup_id_remap}")
            else:
                print("[align] ✅ No subgroup ID mismatches detected - using original IDs")
                self._subgroup_id_remap = None

        except Exception as e:
            print(f"[align] ERROR in ID alignment check: {e}")
            print("[align] Proceeding without remap - this may cause subgroup collapse!")
            self._subgroup_id_remap = None
    ...
    def on_fit_start(self):
        ...
        # Critical: Check subgroup ID alignment between dataset and our mapping
        self._id_alignment_check(ds_prev)
...
#TEST DATALOADER:
        ...
        try:
            ...
            # Helper function to get group name for dataset item
            def get_item_group_name(ds, i):
                """Get group name for dataset item, handling both metadata and fallback."""
                try:
                    if hasattr(ds, 'items_meta') and ds.items_meta and i < len(ds.items_meta):
                        item = ds.items_meta[i]
                        return item.get("group") or item.get("instrument_group") or "unknown"
                    elif hasattr(ds, 'get_meta'):
                        # Lightweight metadata getter if available
                        meta = ds.get_meta(i)
                        return meta.get("group") or meta.get("instrument_group") or "unknown"
                    else:
                        # Fallback to loading from manifest data
                        if i < len(data):
                            item = data[i]
                            return item.get("group") or item.get("instrument_group") or "unknown"
                        return "unknown"
            ...
            # Combined group + within-group subgroup balancing
            alpha, beta = 0.3, 0.7  # Group weight power, subgroup weight power

            # Helper to get subgroup name for dataset item
            def get_item_subgroup_name(ds, i):
                try:
                    if hasattr(ds, 'items_meta') and ds.items_meta and i < len(ds.items_meta):
                        item = ds.items_meta[i]
                        return item.get("sub_group") or item.get("subgroup") or "unknown"
                    elif hasattr(ds, 'get_meta'):
                        meta = ds.get_meta(i)
                        return meta.get("sub_group") or meta.get("subgroup") or "unknown"
                    else:
                        if i < len(data):
                            item = data[i]
                            return item.get("sub_group") or item.get("subgroup") or "unknown"
            ...
            # Count hours per (group, subgroup) combination
            from collections import defaultdict
            hours_subgroup = defaultdict(float)

            # Estimate subgroup hours (simplified - could be made more accurate with real duration data)
            subgroup_counts = defaultdict(int)
            for i in range(min(len(ds), 1000)):  # Sample to estimate distribution
                group_name = get_item_group_name(ds, i)
                subgroup_name = get_item_subgroup_name(ds, i)
                subgroup_counts[(group_name, subgroup_name)] += 1

            # Distribute group hours proportionally to subgroup counts
            for (group, subgroup), count in subgroup_counts.items():
                group_total_count = sum(c for (g, sg), c in subgroup_counts.items() if g == group)
                if group_total_count > 0:
                    proportion = count / group_total_count
                    hours_subgroup[(group, subgroup)] = hours.get(group, 1.0) * proportion

            # Combined weighting: w = (1/hours_group)^α * (1/hours_subgroup_within_group)^β
            w_per_group = {g: (1.0 / (h + 1e-6)) ** alpha for g, h in hours.items()}
            w_per_subgroup = {(g, sg): (1.0 / (h + 1e-6)) ** beta for (g, sg), h in hours_subgroup.items()}

            # Build weights IN THE SAME ORDER AS THE DATASET
            weights = []
            for i in range(len(ds)):
                group_name = get_item_group_name(ds, i)
                subgroup_name = get_item_subgroup_name(ds, i)

                w_g = w_per_group.get(group_name, 1.0)
                w_sg = w_per_subgroup.get((group_name, subgroup_name), 1.0)
                combined_weight = w_g * w_sg
                weights.append(combined_weight)

            # Normalize and apply floor/ceiling
            weights = torch.tensor(weights, dtype=torch.float32)
            weights = weights / weights.mean()  # normalize
            weights = torch.clamp(weights, min=0.6, max=2.0)  # floor 0.6x, ceil 2.0x

            sampler = WeightedRandomSampler(weights=weights, num_samples=len(weights), replacement=True)
            use_weighted_sampling = True

            print(f"[balance] Using combined group+subgroup weighted sampling (α={alpha}, β={beta}, floor=0.6, ceil=2.0)")
            print(f"  Group hours: {dict(hours)}")

            # Show subgroup distribution within groups (top ones)
            print(f"  Subgroup distribution within groups:")
            for group in sorted(hours.keys()):
                group_subgroups = [(sg, h) for (g, sg), h in hours_subgroup.items() if g == group]
                if group_subgroups:
                    group_subgroups.sort(key=lambda x: x[1], reverse=True)
                    top_subs = group_subgroups[:3]  # Show top 3
                    sub_str = ", ".join([f"{sg}:{h:.1f}h" for sg, h in top_subs])
                    print(f"    {group}: {sub_str}")

            print(f"  Final weight range: [{weights.min():.2f}, {weights.max():.2f}] (mean={weights.mean():.2f})")
    ...
    def _preview_x0_direct_rf(self, batch, t_scalar=0.5, sr_out=48000):
        ...
        with self._amp():
            ...
            # controls on device
            # Apply subgroup ID remapping for preview methods too
            subgroup_id_preview = batch["instrument"]["subgroup_id"].to(self.device)
            if hasattr(self, "_subgroup_id_remap") and self._subgroup_id_remap is not None:
                subgroup_id_preview = self._safe_remap(subgroup_id_preview, self._subgroup_id_remap)
            ...
            tokens, mask = self.ctrl_enc(
                ...
                subgroup_id=subgroup_id_preview,
            )
    ...
    @torch.no_grad()
    def _preview_ema_20(self, batch, sr_out=48000):
        """Euler preview with 20 steps using EMA weights."""
        # Early return if preview disabled or memory insufficient
        if not self._can_preview():
            return
        if getattr(self, "_ema_enabled", False):
            with _Swap(self._ema, self.transformers):
                return self._preview(batch, steps=20, sr_out=sr_out, tag="ema_20")
        else:
            return self._preview(batch, steps=20, sr_out=sr_out, tag="no_ema_20")

    @torch.no_grad()
    def _preview_euler_30(self, batch, sr_out=48000):
        """Euler preview with 30 steps using regular weights."""
        # Early return if preview disabled or memory insufficient
        if not self._can_preview():
            return
        return self._preview(batch, steps=30, sr_out=sr_out, tag="euler_30") 
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        if preview_now and self._can_preview():
            try:
                ...
                if self.global_step in (0, 5, 10, 20, 50, 100):
                    # Generate both 30-step and EMA 20-step Euler previews from SAME sample as GT
                    self._preview_euler_30(fixed, sr_out=32000)
                    self._preview_ema_20(fixed, sr_out=32000)
            except Exception as e:
        ...
        if (self.global_step + 1) % max(1, getattr(self.hparams, "every_plot_step", 999999)) == 0 and self._can_preview():
            try:
                # Generate both 30-step and EMA 20-step Euler previews for SAME sample as GT
                self._preview_euler_30(fixed, sr_out=48000)
                self._preview_ema_20(fixed, sr_out=48000)
            except Exception as e:
                print(f"[preview] skipped due to error: {e}")
    ...
    def optimizer_step(self, *args, **kwargs):
        """Override to update EMA weights after each optimizer step."""
        super().optimizer_step(*args, **kwargs)
        # Update EMA only when actual optimizer step happens (not on grad accumulation micro-batches)
        if getattr(self, "_ema_enabled", False):
            self._ema.update()
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if getattr(self, "_ema_enabled", False):
            ...
            print("[EMA] Saved state to checkpoint")
...
# OG ACE STEP DIFFUSION
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Apply subgroup ID remapping for ctrl_enc too (critical!)
        subgroup_id_for_ctrl = batch["instrument"]["subgroup_id"]
        if hasattr(self, "_subgroup_id_remap") and self._subgroup_id_remap is not None:
            subgroup_id_for_ctrl = self._safe_remap(subgroup_id_for_ctrl, self._subgroup_id_remap)
        ...
        tokens, mask = self.ctrl_enc(
            ...
            subgroup_id=subgroup_id_for_ctrl,
        )
        ...
        sub_tgt_raw  = batch["instrument"]["subgroup_id"].long().to(sub_logits.device)

        # Apply subgroup ID remapping if needed (critical for subgroup collapse fix!)
        if hasattr(self, "_subgroup_id_remap") and self._subgroup_id_remap is not None:
            sub_tgt = self._safe_remap(sub_tgt_raw, self._subgroup_id_remap)
        else:
            sub_tgt = sub_tgt_raw
        # Strengthen subgroup signal with separate weights for group vs subgroup
        aux_group_w = 0.2  # Reduce group weight
        aux_sub_w   = 0.6  # Increase subgroup weight significantly
        # Schedule total aux loss weight gradually: start at 0.1, increase to 0.8 over 100k steps
        aux_total_w = min(0.8, 0.1 + (self.global_step / 100000) * 0.7)

        aux_loss = aux_total_w * (
            aux_group_w * F.cross_entropy(group_logits, group_tgt, weight=self.group_ce_w) +
            aux_sub_w   * F.cross_entropy(sub_logits, sub_tgt, weight=self.sub_ce_w)
        )
        ...
        with torch.no_grad():
            ...
            self.log("breakdown/winds_frac", (group_tgt==self.group2id.get("winds",-1)).float().mean(), on_step=True)
            self.log("breakdown/piano_frac", (group_tgt==self.group2id.get("piano",-1)).float().mean(), on_step=True)
        ...
        if disable:
            ...
            outlier_mode = 0  # 0 = disabled
        else:
            # Per-group outlier thresholding
            q = float(getattr(self, "outlier_quantile", 0.90))
            q = min(max(q, 0.50), 0.99)
            keep_mask = torch.ones_like(recon_per_ex)
            outlier_mode = 1  # 1 = per-group

            # Track per-group filtering stats
            group_stats = {}
            for gid in torch.unique(group_tgt):
                m = (group_tgt == gid)
                if m.any():
                    thr = torch.quantile(recon_per_ex[m].detach().float(), q)
                    group_keep = (recon_per_ex[m] <= thr).float()
                    keep_mask[m] = group_keep

                    # Log per-group filtering stats
                    group_name = [name for name, id in self.group2id.items() if id == gid.item()]
                    if group_name:
                        group_name = group_name[0]
                        kept_count = int(group_keep.sum().item())
                        total_count = int(m.sum().item())
                        group_stats[group_name] = f"{kept_count}/{total_count}"

            # Log group filtering stats occasionally
            if self.global_step % 100 == 0:
                stats_str = ", ".join([f"{g}:{s}" for g, s in group_stats.items()])
                if stats_str:
                    print(f"[step {self.global_step}] Per-group outlier filtering (q={q:.2f}): {stats_str}")

        kept    = int(keep_mask.sum().item())
        dropped = int(keep_mask.numel() - kept)
        recon_loss = (recon_per_ex * keep_mask).sum() / keep_mask.clamp_min(1).sum()

        loss = recon_loss + self.adapter_l2 * cond_reg + aux_loss + self.pr_loss_weight * pr_loss

        v_target = z - x0
        v_loss   = F.mse_loss(v_pred, v_target)  # diag only

        # Log separate group and subgroup CE losses for monitoring
        group_ce_loss = F.cross_entropy(group_logits, group_tgt, weight=self.group_ce_w)
        sub_ce_loss = F.cross_entropy(sub_logits, sub_tgt, weight=self.sub_ce_w)

        self.log("aux/total", aux_loss.detach(), on_step=True)
        self.log("aux/group_ce", group_ce_loss.detach(), on_step=True)
        self.log("aux/sub_ce", sub_ce_loss.detach(), on_step=True)
        self.log("aux/group_weight", aux_group_w, on_step=True)
        self.log("aux/sub_weight", aux_sub_w, on_step=True)
        self.log("aux/total_weight", aux_total_w, on_step=True)
        self.log("train/outlier_mode", torch.tensor(outlier_mode, device=loss.device), on_step=True)
...
# NEW METHOD
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Calculate instrument classification accuracy (with full ctrl_enc signature)
        # Apply subgroup ID remapping for validation too
        subgroup_id_for_val = batch["instrument"]["subgroup_id"]
        if hasattr(self, "_subgroup_id_remap") and self._subgroup_id_remap is not None:
            subgroup_id_for_val = self._safe_remap(subgroup_id_for_val, self._subgroup_id_remap)
        ...
        tokens, mask = self.ctrl_enc(
            ...
            group_id=batch["instrument"]["group_id"],
            subgroup_id=subgroup_id_for_val,
        )
        inst_tok = tokens[:, 0, :]
        ...
        # Subgroup classification accuracy (use remapped targets)
        sub_logits = self.sub_head(inst_tok)
        sub_pred = sub_logits.argmax(-1)
        sub_tgt_raw = batch["instrument"]["subgroup_id"].long().to(sub_logits.device)
        # Apply same remapping as in training
        if hasattr(self, "_subgroup_id_remap") and self._subgroup_id_remap is not None:
            sub_tgt = self._safe_remap(sub_tgt_raw, self._subgroup_id_remap)
        else:
            sub_tgt = sub_tgt_raw
