...
# === CONFIG ===
# 1) Add FORCE switch at top
FORCE = False  # set True to recompute everything
...
MAX_WORKERS = min(12, cpu_count())  # Use up to 12 cores
...
def render_drum_bus_for_group(session_name, group_idx, group_files, loaded_by_name, role_idx):
    ...
    # 3) Skip bus/stems if already there
    ...
    # If everything already rendered and not forcing, skip early
    if not FORCE:
        stem_files_exist = all([
            (stem_dir / "kick_direct.wav").exists(),
            (stem_dir / "snare_direct.wav").exists(),
            (stem_dir / "toms_sum.wav").exists(),
            (stem_dir / "hats_ride.wav").exists(),
            (stem_dir / "cymbals.wav").exists(),
            (stem_dir / "oh_room.wav").exists()
        ])

        if bus_path.exists() and stem_files_exist:
            print(f"   ⏭️  drum bus/stems exist, skipping render -> {bus_path}")
            sys.stdout.flush()
            return
    ...
    # 8) Write files using atomic writes
    safe_write_wav(bus_path, bus, FS)
    for name, arr in [
        ("kick_direct.wav", peak_normalize(kick_direct)),
        ("snare_direct.wav", peak_normalize(snare_direct)),
        ("toms_sum.wav", peak_normalize(toms_sum)),
        ("hats_ride.wav", peak_normalize(hats_ride)),
        ("cymbals.wav", peak_normalize(cymbals)),
        ("oh_room.wav", peak_normalize(oh_room)),
    ]:
        safe_write_wav(stem_dir / name, arr, FS)
...
# === MAIN ===
def get_drum_files_for_session(all_paths, target_session_name):
    """Get drum files for a specific session (optimized)"""
    drum_files = []
    seen_paths = set()

    # Quick scan focusing only on target session paths
    for path_str in all_paths:
        # Quick session name check before expensive operations
        if target_session_name not in path_str:
            continue
        ...
        # Skip duration and silence checks if file doesn't exist
        if path.exists():
    ...
    # Final deduplication by filename
    seen_filenames = set()
    unique_drum_files = []
    for file_info in drum_files:
...
def get_session_names_fast(all_paths):
    """Quickly get unique session names without full processing"""
    sessions = set()

    for path_str in all_paths:
        # Quick check for drum files before parsing
        filename = Path(path_str).name.lower()
        if not any(kw in filename for kw in ["kick", "snare", "hihat", "tom", "oh", "drum"]):
            continue

        # Extract session name from path string directly (faster than Path.parts)
        if "/New/" in path_str:
            try:
                session_name = path_str.split("/New/")[1].split("/")[0]
                sessions.add(session_name)
            except IndexError:
                continue
        elif "/Prev/" in path_str:
            try:
                session_name = path_str.split("/Prev/")[1].split("/")[0]
                sessions.add(session_name)
            except IndexError:
                continue
...
def process_single_session(session_name, all_paths):
    ...
    try:
        ...
        # Get drum files for this session (fast lookup)
        drum_files = get_drum_files_for_session(all_paths, session_name)
        ...
        # 4) Skip whole session if onsets OR drum mixes already exist
        has_onsets_or_drums = True
        for gi, group in enumerate(groups):
            # Check if this group has onsets OR drum mixes
            group_has_onsets = True
            group_has_drums = True

            # Check onsets for all files in group
            for f in group:
                p = Path(f['path'])
                base = OUTPUT_DIR / session_name / p.stem
                if not base.with_suffix(".onsets.npy").exists():
                    group_has_onsets = False
                    break

            # Check drum bus/stems for this group
            bus_path = BUS_DIR / session_name / f"group_{gi+1:02d}.bus.wav"
            stems_ok = all((BUS_DIR / session_name / f"group_{gi+1:02d}_stems" / s).exists()
                           for s in ["kick_direct.wav","snare_direct.wav","toms_sum.wav","hats_ride.wav","cymbals.wav","oh_room.wav"])
            if not (bus_path.exists() and stems_ok):
                group_has_drums = False

            # If this group doesn't have onsets AND doesn't have drums, we need to process
            if not (group_has_onsets or group_has_drums):
                has_onsets_or_drums = False
                break

        if has_onsets_or_drums and not FORCE:
            print(f"⏭️  Session {session_name} already has onsets or drum mixes, skipping.")
            sys.stdout.flush()
    ...
    for group_idx, group in enumerate(groups):
            ...
            # 2) Skip events if already there
            evt_path = EVENT_JSON / session_name / f"group_{group_idx+1:02d}.events.json"
            evt_path.parent.mkdir(parents=True, exist_ok=True)

            if evt_path.exists() and not FORCE:
                print(f"   ⏭️  events exist, skipping -> {evt_path}")
                sys.stdout.flush()
                # still record in group_data for the summary
                group_data['events_file'] = str(evt_path)
                with open(evt_path, "r") as jf:
                    try:
                        prev = json.load(jf)
                        group_data['num_events'] = len(prev)
                    except Exception:
                        group_data['num_events'] = None
            else:
                # ... build events as you do now ...
                # atomic write
                tmp = evt_path.with_suffix(".tmp")
                with open(tmp, "w") as jf:
                    json.dump(events, jf, indent=2)
                os.replace(tmp, evt_path)
...
def worker_process_session(args):
    """Worker function for multiprocessing"""
    session_name, all_paths = args
    try:
        return process_single_session(session_name, all_paths)
    except Exception as e:
        print(f"   ❌ Error processing {session_name}: {str(e)}")
        return session_name, None, 0

if __name__ == "__main__":
    # 5) (Nice-to-have) --force CLI flag
    parser = argparse.ArgumentParser()
    parser.add_argument("--force", action="store_true", help="Force recomputation of all outputs")
    args = parser.parse_args()
    FORCE = args.force

    all_paths = load_paths()

    # Get session names quickly
    print("🔍 Discovering sessions with drum files...")
    session_names = get_session_names_fast(all_paths)
    print(f"📁 Found {len(session_names)} sessions with drum files")

    # Process sessions in parallel using multiprocessing
    print(f"🚀 Processing {len(session_names)} sessions using {MAX_WORKERS} CPU cores...")

    # Prepare arguments for worker processes
    worker_args = [(session_name, all_paths) for session_name in session_names]
    ...
    with Pool(processes=MAX_WORKERS) as pool:
        ...
        session_results = pool.map(worker_process_session, worker_args)

        # Collect results
        for session_name_result, session_groups, session_group_count in session_results:
            if session_groups is not None:
                result[session_name_result] = session_groups
                total_groups += session_group_count
                print(f"✅ Completed {session_name_result}: {session_group_count} groups")
            else:
                print(f"❌ Skipped {session_name_result}: processing failed")

    print(f"\n📈 Final: {len(result)} sessions completed ({total_groups} groups total)")
    sys.stdout.flush()
