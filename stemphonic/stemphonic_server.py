...
def _run_demucs_separation(task_id, audio_path):
    """Worker: separate the input audio into stems + cache each as a latent.

    FAST PATH: latent demucs student (`/scratch/latent_demucs/`) maps
    waveform → 4 stem latents (drums, bass, vocals, other) in a SINGLE
    forward pass. ~5× faster than htdemucs+per-stem-encode and the
    output is already in VAE latent space (no re-encode needed).

    LEGACY FALLBACK: htdemucs_6s + per-stem soundfile write + per-stem
    vae.encode. Used when the student model file is missing or fails
    to load.
    """
    try:
        ...
        # ── Only path: latent_demucs + latent_drumsep (strict, no fallback) ──
        # Backend NEVER decodes latents to audio. Output is latent-only:
        # the frontend's WebGPU ONNX VAE decoder is the playback path.
        student = _get_latent_demucs()
        logger.info("[%s] latent_demucs path (audio-in, latents-out, no decode)", task_id)
        wav_np, sr_in = sf.read(audio_path, dtype="float32", always_2d=True)
        stem_lats = student.separate(wav_np, sr_in)  # {name: [T, 64]}
        for name, L in stem_lats.items():
            meta = _save_raw_latent(L, stem_type=name, source_path=None)
            stem_latents[name] = meta
            logger.info("  latent_demucs %s → latent %s (%d frames)",
                        name, meta["latent_id"], meta["n_frames"])
        DEMUCS_TASKS[task_id]["stems"] = stem_urls  # empty — no wavs, playback via /api/latent
        DEMUCS_TASKS[task_id]["stem_latents"] = stem_latents
        DEMUCS_TASKS[task_id]["separator"] = "latent_demucs"

        # Latent-native drum sub-separation: drum latent → latent_drumsep
        # → 6 sub-stem latents, persisted via _save_raw_latent. No VAE
        # decode, no wav roundtrip.
        if "drums" in stem_lats:
            try:
                drum_subs = _drumsep_and_encode_substems(
                    drums_latent=stem_lats["drums"],
                )
                DEMUCS_TASKS[task_id]["drum_substem_latents"] = drum_subs
                logger.info("  latent_drumsep substems: %s",
                            list(drum_subs.keys()))
            except Exception as ds_err:
                traceback.print_exc()
                logger.warning("  latent_drumsep failed: %s", ds_err)

        DEMUCS_TASKS[task_id]["status"] = "completed"
        logger.info("✅ latent_demucs task %s done: %s",
                    task_id, list(stem_latents.keys()))
    except Exception as e:
        traceback.print_exc()
